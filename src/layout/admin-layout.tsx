...
export default function AdminLayout({
    ...
    const { isCollapsed, setIsCollapsed, permission, urlGeralAdm } = useContext(UserContext)
    ...
    return (
        <div>
            <SidebarProvider defaultOpen={true} open={isCollapsed} onOpenChange={() => setIsCollapsed((prev) => !prev)}>
                <AppSidebarAdmin />
                                ...
                                <Breadcrumb>
                                    <BreadcrumbList>
                                        {breadcrumbItems.map((segment, index) => {
                                            ...
                                            const href = index === 0 ? '/' : `/${pathSegments.slice(0, index + 1).join('/')}`;
                                            return (
