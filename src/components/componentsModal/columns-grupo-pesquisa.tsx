...
export const columns: ColumnDef<PesquisadorProps>[] = [
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-3 items-center"> {row.getValue('name')}</div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex w-fit gap-1  ">{row.getValue('first_leader')}</div>
      );
    },
  },
  {
    ...
    header: () => (
      <div className="text-right flex items-center">Vice-líder</div>
    ),
    cell: ({ row }) => {
      return (
        <div className="flex w-fit gap-1  ">
          {row.getValue('second_leader')}
        </div>
      );
    },
  },
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex w-fit gap-1 items-center  ">
          {' '}
          <div
            className={`w-4 min-w-4 h-4 rounded-md dark:border-neutral-800 border  border-neutral-200  ${qualisColor[normalizeArea(row.original.area || '')]} min-h-full relative`}
          ></div>
          {row.getValue('area')}
        </div>
      );
...

  {
    ...
    header: () => (
      <div className="text-right flex items-center">Instituição</div>
    ),
    cell: ({ row }) => {
      return (
        <div className="flex w-fit gap-1   ">{row.getValue('institution')}</div>
      );
    },
  },
];
