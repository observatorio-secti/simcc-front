...
import { ArrowRight, ChevronLeft, Factory, Info, Sprout, Tractor  } from "lucide-react";
...
const incitesList: Incite[] = [
    {
        ...
        description: "Explore o panorama da Indústria 4.0 na Bahia, com dados sobre maturidade digital, tecnologias e oportunidades para o setor.",
    ...
    },
    {
        ...
        description: "Informações estratégicas sobre inovação tecnológica no setor agropecuário da Bahia.",
    ...
    },
    {
        ...
        description: "Informações estratégicas sobre inovação tecnológica no setor agroindustrial da Bahia.",
...
export function IncitesPage() {
    ...
    const renderInciteCard = (incite: Incite) => {
            ...
            >
                ...
                <div className="relative z-10 flex flex-col justify-between h-full">
                    <div>
                        ...
                        <div className="mb-4">
                            <incite.icon className={`h-8 w-8 ${isComingSoon ? 'text-neutral-400' : 'text-white'}`} />
                        </div>
                        <h3 className="text-xl font-bold tracking-tight text-white">{incite.title}</h3>
                        <p className="mt-2 text-sm text-neutral-300">{incite.description}</p>
                    </div>
                    {!isComingSoon && (
        ...
        if (isComingSoon) {
            return <div key={incite.id} className="opacity-60 cursor-not-allowed">{cardContent}</div>;
        }

        return (
            <Link to={incite.link} target="_blank" rel="noopener noreferrer" key={incite.id} className="no-underline">
                {cardContent}
            </Link>
        );
    };

    return <main className="relative flex flex-col items-end justify-center min-h-screen p-4 md:p-8 bg-cover bg-center" style={{ backgroundImage: `url(${bg_popup})` }} >
        ...
        <div className="relative z-10 w-full max-w-6xl">
            <Helmet>
                ...
                <meta name="description" content="Explore os painéis interativos com dados e informações estratégicas sobre os setores produtivos da Bahia." />
            ...
            <div className="flex items-center gap-6 mb-8 justify-end">
                <div className="flex-1 text-right">
                    ...
                    <p className="mt-2 text-neutral-600 dark:text-neutral-400 max-w-3xl ml-auto">
                        Plataformas interativas que apresentam diagnósticos e informações estratégicas sobre setores produtivos relevantes para o desenvolvimento da Bahia.
                    </p>
                </div>
            </div>
