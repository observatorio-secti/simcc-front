...
import { Blocks, MapPin, User } from 'lucide-react';
...
export function ProgramItem(props: GraduateProgram) {
  ...
  const handlePesquisaFinal = () => {
    ...
    // Se veio da página de instituição, preserva o contexto
    if (props.fromInstitution && props.institutionId && props.institutionName) {
      queryUrl.set('from_institution', 'true');
      queryUrl.set('institution_id', props.institutionId);
      queryUrl.set('institution_name', props.institutionName);
    }
  ...
  return (
    ...
    >
      <Alert
        className={`rounded-r-none border-r-0 w-2 min-w-2 p-0 ${getColorByArea(props.area)}`}
      />
    ...
    ${
      Number(props.rating) <= 2
        ? 'bg-gray-500'
        : Number(props.rating) <= 4
          ? 'bg-yellow-500'
          : Number(props.rating)
            ? 'bg-green-500'
            : 'bg-blue-500'
        ...
        <div className="flex justify-between gap-8 items-start">
          <div className="flex gap-3 w-full">
            <div className="w-full">
              ...
              <div>
                {props.acronym && (
                  <p className="text-gray-500 text-sm">{props.acronym}</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="mt-4 flex gap-2 items-center">
          ...
          {props.researchers.length > 5 && (
            <p className="h-8 w-8 flex items-center justify-center text-gray-500">
              +
            </p>
          )}
        </div>
