...
export function FiltersModal({
  ...
  const uniqueAreas = Array.from(
    new Set(
      graduatePrograms.flatMap((res) =>
        res.area.split(';').map((area) => area.trim()),
      ),
    ),
  );
  const uniqueCities = Array.from(
    new Set(graduatePrograms.map((res) => res.city)),
  ).filter(Boolean);
  const uniqueUniversities = Array.from(
    new Set(graduatePrograms.map((res) => res.institution)),
  ).filter(Boolean);
  const uniqueTypes = Array.from(
    new Set(graduatePrograms.map((res) => res.type)),
  ).filter(Boolean);
  const uniqueModalities = Array.from(
    new Set(graduatePrograms.map((res) => res.modality)),
  ).filter(Boolean);
  ...
  useEffect(() => {
    ...
    if (selectedUniversities.length > 0) {
      filtered = filtered.filter((r) =>
        selectedUniversities.includes(r.institution),
      );
    }
    ...
    if (selectedModalities.length > 0) {
      filtered = filtered.filter((r) =>
        selectedModalities.includes(r.modality),
      );
    }
  ...
  const filteredPrograms = graduatePrograms.filter((res) => {
    const hasSelectedArea =
      selectedAreas.length === 0 || selectedAreas.includes(res.area);

    const hasSelectedCity =
      selectedCities.length === 0 ||
      selectedCities.includes(normalizeString(res.city));
    const hasSelectedUniversity =
      selectedUniversities.length === 0 ||
      selectedUniversities.includes(res.institution);
    const hasSelectedType =
      selectedTypes.length === 0 || selectedTypes.includes(res.type);
    const hasSelectedModality =
      selectedModalities.length === 0 ||
      selectedModalities.includes(res.modality);
  ...
  const filteredTotal = Array.isArray(uniqueAreas)
    ? uniqueAreas.filter((item) => {
                      ...
                      >
                        {filteredTotal.map((area) => (
                          <ToggleGroupItem
                            key={area}
                            value={area}
                            className="px-3 gap-2 flex py-2"
                          >
                            <Alert
                              className={` w-4 rounded-md border-0 h-4 p-0 ${getColorByArea(area)}`}
                            />{' '}
                            {area}
                          </ToggleGroupItem>
                        ))}
...
export function GraduateProgram() {
  ...
  const filtrosAplicadosRef = useRef<HTMLDivElement>(null);

  // Função para rolar até a seção de filtros aplicados
  const scrollToFiltrosAplicados = () => {
    if (filtrosAplicadosRef.current) {
      filtrosAplicadosRef.current.scrollIntoView({
        behavior: 'smooth',
        block: 'start',
      });
    }
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        if (data) {
          ...
          setOriginalGraduatePrograms(
            data.filter((item) => item.visible == true),
          );
  ...
  // Função personalizada para definir cidades selecionadas e rolar para filtros
  const handleSetSelectedCities = (
    cities: string[] | ((prev: string[]) => string[]),
  ) => {
    setSelectedCities(cities);
    // Aguarda um pequeno delay para garantir que o estado seja atualizado antes da rolagem
    setTimeout(() => {
      scrollToFiltrosAplicados();
    }, 100);
  };
  ...
  return (
    <>
      <Helmet>
        <title>Pós-graduações | {'Simcc'}</title>
        <meta name="description" content={`Pós-graduações | ${'Simcc'}`} />
            ...
            <main className="z-[2]  gap-4 md:gap-8 flex flex-col  pt-0 md:pt-0 w-full">
              ...
              <div>
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="hidden xl:flex xl:flex-nowrap gap-2">
                        ...
                        <div>
                          <Button
                            onClick={() => onOpen('filters-graduate')}
                            variant="ghost"
                            className=""
                          >
                ...
                <div className="mt-8 px-4 md:px-8">
                  <div
                    className={`${selectedAreas.length > 0 || selectedCities.length > 0 || selectedModalities.length > 0 || selectedTypes.length > 0 || selectedUniversities.length > 0 ? 'flex' : 'hidden'} flex flex-wrap gap-3 mb-6 items-center`}
                  >
                    ...
                    {selectedCities.map((item) => (
                      ...
                      >
                        ...
                        <div
                          ...
                          onClick={() =>
                            setSelectedCities(
                              selectedCities.filter((i) => i !== item),
                            )
                          }
                        >
                          <X size={16} />
                    ...
                    {selectedUniversities.map((item) => (
                      ...
                      >
                        ...
                        <div
                          ...
                          onClick={() =>
                            setSelectedUniversities(
                              selectedUniversities.filter((i) => i !== item),
                            )
                          }
                        >
                          <X size={16} />
                    ...
                    {selectedModalities.map((item) => (
                      ...
                      >
                        ...
                        <div
                          ...
                          onClick={() =>
                            setSelectedModalities(
                              selectedModalities.filter((i) => i !== item),
                            )
                          }
                        >
                          <X size={16} />
