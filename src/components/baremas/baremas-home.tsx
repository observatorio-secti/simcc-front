...
export function BaremasHome() {
  ...
  const [tituloBarema, setTituloBarema] = useState(
    'Barema de avaliação sem título',
  );
        ...
        try {
          if (user) {
            ...
            if (userDocSnapshot.size > 0) {
              const userDataArray: Firebase[] = userDocSnapshot.docs.map(
                (doc) => {
                  // Get the document ID using .id
                  const documentId = doc.id;
                  // Combine document ID with document data
                  const userData = {
                    ...(doc.data() as Firebase),
              ...
              setPesquisadoresSelecionados(
                userData.pesquisadores as Research[],
              );
  ...
  const CRITERIOS: Criterio2[] = [
    ...
    {
      id: 5,
      value: 'Artigo em periódicos qualificados (A a C)',
      type: 'Artigos',
    },
    ...
    {
      id: 14,
      value: 'Pós-Graduação Stricto Sensu',
      type: 'Formação de recursos humanos em andamento',
    },
    {
      id: 15,
...

    {
      id: 31,
      value: 'Pós-Graduação Stricto Sensu',
      type: 'Formação de recursos humanos concluída',
    },
    {
      id: 16,
      value: 'Iniciação Científica',
    ...
    { id: 33, value: 'Organização', type: 'Participação em eventos' },
    {
      id: 134,
      value:
        'Palestrante/ Mesa Redonda/ Conferencista/ Ministrante de Minicurso',
  ...
  const baixarCSV = (grupos: Grupo[]) => {
    ...
    const categorias = grupos.flatMap((grupo) =>
      grupo.categorias.map((categoria) => categoria.criterio),
    );
      ...
      .forEach((pesquisador) => {
        ...
        const valoresCategorias = grupos.flatMap((grupo) =>
          grupo.categorias.map((categoria) => {
            const pesquisadorData = categoria.pesquisadores.find(
              (p) => p.id === pesquisador.id,
            );
            return pesquisadorData
              ? parseFloat(pesquisadorData.total.toString()).toFixed(2)
              : '0.00';
          }),
        );
  ...
  const deletarGrupo = (grupoIndex: number) => {
    ...
    novosOpenPopovers.splice(
      grupoIndex * novosGrupos[0].categorias.length,
      novosGrupos[0].categorias.length,
    );
  ...
  const duplicarGrupo = (grupoIndex: any) => {
    ...
    novosOpenPopovers.splice(
      posicaoInsercao,
      0,
      ...Array(grupoDuplicado.categorias.length).fill(false),
    );
    setOpenPopovers(novosOpenPopovers);
  };
  ...
  const formatResearcherData = (
    p: PesquisadoresSelecionados,
    idCriterio: number,
  ) => ({
    total: 0,
    id: p.id,
    ...
    const novosGrupos = grupos.map((grupo, index) => {
      if (index === grupoIndex) {
        const novasCategorias = grupo.categorias.map((categoria, idx) => {
          ...
          const pesquisadoresFiltradosOutros = pesquisadores
            .filter(
              (pesquisador) =>
                pesquisador.graduation.toUpperCase() !==
                criterioItem.toUpperCase(),
          ...
          if (idx === categoriaIndex) {
            return {
              ...
              pesquisadores: [1, 2, 3, 4].includes(criterioId)
                ? pesquisadoresFiltradosTitulacao
                : pesquisadoresFiltradosOutros,
            };
          }
          return categoria;
        });
  ...
  const toggleEditTitulo = (grupoIndex: any, type: any) => {
    if (type === 'titulo') {
      setEditingGrupoIndex(
        grupoIndex === editingGrupoIndex ? null : grupoIndex,
      );
  ...
  const config = [
    ...
    {
      id: 2,
      itens: 'Artigos completos em anais de eventos',
      valueSetter: setAnoWorkInEvent,
      value: anoWorkInEvent,
    },
    { id: 3, itens: 'Livros', valueSetter: setAnoLivro, value: anoLivro },
    {
      id: 4,
      itens: 'Capítulos de livro',
      valueSetter: setAnoCapLivro,
      value: anoCapLivro,
    },
    ...
    { id: 6, itens: 'Marca', valueSetter: setAnoMarca, value: anoMarca },
    {
      id: 7,
      itens: 'Software',
      valueSetter: setAnoSoftware,
      value: anoSoftware,
    },
    {
      id: 8,
      itens: 'Formação em andamento',
      valueSetter: setAnoResourceProgess,
      value: anoResourceProgess,
    },
    {
      id: 9,
      itens: 'Formação concluída',
      valueSetter: setAnoResourceCompleted,
      value: anoResourceCompleted,
    },
    {
      id: 8,
      itens: 'Participação em eventos',
      valueSetter: setAnoParticipacao,
      value: anoParticipacao,
    },
  ];
  ...
  const calcularSomaPontuacaoMaxima = (grupo: any) => {
    return grupo.categorias.reduce(
      (total, categoria) => total + parseInt(categoria.pontuacao_max),
      0,
    );
  };
  ...
  const handleResearcherUpdate = (newResearcherData: PesquisadorUpdate) => {
    const updatedGrupos = grupos.map((grupo) => {
      const updatedCategorias = grupo.categorias.map((categoria) => {
        ...
        const updatedPesquisadores = categoria.pesquisadores.map(
          (pesquisador) => {
    ...
    grupos.forEach((grupo) => {
      grupo.categorias.forEach((categoria: any) => {
        categoria.pesquisadores.forEach((pesquisador: any) => {
          ...
          const pesquisadorIndex = somaTotalPorPesquisador.findIndex(
            (p: any) => p.id === id,
          );

          if (pesquisadorIndex === -1) {
            // Se o pesquisador não existe, adiciona-se um novo
            somaTotalPorPesquisador.push({
              id,
              name,
              total: totalAtualizado,
              grupos: [{ titulo: grupo.titulo, total: totalAtualizado }],
            });
          } else {
            // Se o pesquisador já existe, verifica-se se o grupo já existe no array de grupos
            const grupoIndex = somaTotalPorPesquisador[
              pesquisadorIndex
            ].grupos.findIndex((g: any) => g.titulo === grupo.titulo);
            if (grupoIndex === -1) {
              // Se o grupo não existe, adiciona-se um novo
              somaTotalPorPesquisador[pesquisadorIndex].grupos.push({
                titulo: grupo.titulo,
                total: totalAtualizado,
              });
            } else {
              // Se o grupo já existe, soma-se o total
              somaTotalPorPesquisador[pesquisadorIndex].grupos[
                grupoIndex
              ].total += totalAtualizado;
            }
  ...
  const [researcherSelecionados, setResearcherSelecionados] = useState<
    Research[]
  >([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const names = pesquisadoresSelecionados
          .map((pesquisador) => pesquisador.name)
          .join(';');
  ...
  return (
    <>
      <Helmet>
        <title>Baremas | Módulo administrativo | {'Simcc'} </title>
        <meta
          name="description"
          content={`Baremas | Módulo administrativo | ${'Simcc'}`}
        />
            ...
            >
              <div>
                ...
                <p className="max-w-[750px]  text-lg font-light text-foreground">
                  Atualize os dados importando o arquivo .xls na plataforma{' '}
                </p>
                <div className="flex gap-3 mt-3">
                  <Button size={'sm'}>Importar dados dos docentes</Button>
                  <Button size={'sm'} variant={'ghost'}>
                    Importar bolsistas CNPq
                  </Button>
                </div>
              </div>
            </TabsContent>
              ...
              <div className="flex">
                <Tabs defaultValue="1" value={valueTab} className="w-full">
                  <div className="flex flex-col gap-8 w-full">
                      ...
                      <Alert className="rounded-t-none ">
                        <div className="flex justify-between  mb-6">
                          <div className="flex flex-col ">
                            {editTituloBarema ? (
                              <div className="flex items-center gap-3 mb-2 ">
                                <Input
                                  ...
                                  className="flex-1"
                                  onChange={(e) =>
                                    setTituloBarema(e.target.value)
                                  }
                                />
                                <Button
                                  variant={'ghost'}
                                  ...
                                  onClick={() =>
                                    setEditTituloBarema(!editTituloBarema)
                                  }
                                >
                                  <Check size={16} />
                                </Button>
                              ...
                              <div className="flex items-center gap-3 mb-2 ">
                                <h1 className=" text-xl  font-medium max-w-[380px] bg-red-700 text-white">
                                  {' '}
                                  {tituloBarema}
                                </h1>
                                <Button
                            ...
                            <p className="mt-2 max-w-[500px] text-gray-500 text-xs dark:text-gray-300">
                              Estas configurações incluem o nome do barema para
                              exportação, o período de ano a ser considerado
                              para a análise e outras definições.
                            </p>
                          </div>

                          <div className="flex gap-3">
                            <Button onClick={() => handleSunmitBarema()}>
                              <Check size={16} />
                              Salvar alterações
                            </Button>
                          </div>
                    ...
                    <TabsContent
                      value="1"
                      className="w-full flex gap-8 flex-col m-0 p-0"
                    >
                                  ...
                                  <div className="gap-4 grid grid-cols-2 ">
                                    {config.map((props) => {
                                      return (
                                        ...
                                        >
                                          <div
                                            className={` gap-3 transition-all flex items-center  rounded-md text-xs font-medium `}
                                          >
                          ...
                          <div className="w-full flex">
                            ...
                            <div className="w-full">
                              <Alert className="rounded-l-none rounded-b-none">
                                <div className="w-full flex items-center justify-center cursor-pointer text-gray-500">
                                  <DotsSix size={20} />
                                </div>
                                <div className="flex justify-between  mb-6">
                                      ...
                                      {editingGrupoIndex === grupoIndex ? ( // Renderizar input de edição se o grupo estiver em modo de edição
                                        <>
                                          <Input
                                            ...
                                            className="flex-1"
                                            onChange={(e) =>
                                              handleTituloChange(e, grupoIndex)
                                            }
                                          />
                                          <Button
                                            variant={'ghost'}
                                  ...
                                  {grupo.categorias.length !== 0 && (
                                    <TableHeader>
                                      <TableRow>
                                        ...
                                        <TableHead className="text-right w-ful">
                                          Total
                                        </TableHead>
                                      </TableRow>
                                    </TableHeader>
                                      ...
                                      grupo.categorias.map(
                                        (categoria, index) => (
                                          <TableRow className="relative">
                                              ...
                                              <div className="">
                                                ...
                                                >
                                                  ...
                                                  <DialogContent className="z-[9999]">
                                                    ...
                                                    <div
                                                      className={
                                                        'max-h-[350px] overflow-y-auto elementBarra'
                                                      }
                                                    >
                                            ...
                                            <TableCell>
                                              <Input
                                                className={`w-24 ${Number(categoria.pontos) > Number(categoria.pontuacao_max) && 'border-red-500'}`}
                                                name="pontuacao_max"
                                                value={categoria.pontuacao_max}
                                                onChange={(e) =>
                                                  handleInputChange(
                                                    e,
                                                    index,
                                                ...
                                                <TooltipProvider>
                                                  <Tooltip>
                                                    <TooltipTrigger asChild>
                                                      <Button
                                                        className="group-hover:flex whitespace-nowrap absolute right-0"
                                                        variant={'ghost'}
                                                        size={'icon'}
                                                        onClick={() =>
                                                          removerCriterio(
                              ...
                              <Alert className="rounded-l-none rounded-t-none border-t-0 dark:bg-neutral-800 bg-neutral-100 flex justify-between items-center">
                                  ...
                                  {grupo.categorias.length !== 0 && (
                                    <div className="flex items-center gap-3">
                                      <p className="text-xs whitespace-nowrap ">
                                        Subtotal (o máximo a ser considerado é
                                      </p>
                                      <Input
                                        className={`w-16 ${calcularSomaPontuacaoMaxima(grupo) > grupo.quantidade_max_pontos && 'border-red-500'}`}
                                        value={grupo.quantidade_max_pontos}
                                        onChange={(e) =>
                                          editarQuantidadeMaxPontos(
                                            grupoIndex,
                                            e.target.value,
                                          )
                                        }
                                ...
                                <div className="flex gap-3 items-center">
                                  ...
                                  <Button
                                    ...
                                    className="hover:bg-neutral-200 dark:hover:bg-neutral-700"
                                    size={'icon'}
                                    onClick={() => deletarGrupo(grupoIndex)}
                                  >
                                    <Trash size={16} />{' '}
                                  </Button>
                    ...
                    <TabsContent
                      value="2"
                      className="w-full flex gap-3 flex-col m-0 p-0"
                    >
                      <div className="w-full flex">
                          ...
                          <div className="flex justify-between  mb-6">
                            <div className="flex flex-col ">
                              <div className="flex items-center gap-3 mb-2 mt-4 ">
                                ...
                                <p className="text-sm font-bold">
                                  {pesquisadoresSelecionados.length}{' '}
                                  {pesquisadoresSelecionados.length != 1
                                    ? 'pesquisadores'
                                    : 'pesquisador(a)'}
                                </p>
                              </div>
                              <p className="mt-2 max-w-[500px] text-gray-500 text-xs dark:text-gray-300">
                                Estas configurações incluem o nome do barema
                                para exportação, o período de ano a ser
                                considerado para a análise e outras definições.
                              </p>
                            </div>
                          </div>
                        </Alert>
                          ...
                          <div className="flex justify-between  mb-6">
                            <div className="flex flex-col ">
                              <div className="flex items-center gap-3 mb-2 mt-4 ">
                                ...
                                <p className="text-sm font-bold">
                                  Resultado e classificação dos pesquisadores
                                </p>
                              </div>
                              <p className="mt-2 max-w-[500px] text-gray-500 text-xs dark:text-gray-300">
                                Soma de todas as pontuações dos pesquisadores
                                por grupo.
                              </p>
                            </div>
                                      ...
                                      >
                                        ...
                                        {grupos.map((grupo) => {
                                          const totalPorColuna =
                                            grupo.categorias.reduce(
                                              (acc, categoria) => {
                                                return (
                                                  acc +
                                                  categoria.pesquisadores.reduce(
                                                    (subAcc, pesquisador) => {
                                                      if (
                                                        pesquisador.id ===
                                                        grupoOrdenado.id
                      ...
                      <div className="w-full flex">
                        ...
                        <Alert className="rounded-l-none ">
                          ...
                          <div>
                            <GraficoBarema
                              pesquisadores={Object.values(
                                somaTotalPorPesquisador,
                              )
                                .map((grupo) => grupo)
                                .sort(
                                  (a, b) =>
