...
export function GruposPesquisaPage() {
  ...
  const institutions = Array.isArray(total)
    ? [
        ...new Set(
          total.map((item) => item.institution).filter((i): i is string => !!i),
        ),
      ]
    : [];
  const areas = Array.isArray(total)
    ? [
        ...new Set(
          total.map((item) => item.area).filter((a): a is string => !!a),
        ),
      ]
    : [];
  ...
  const filteredTotal3 = Array.isArray(institutions)
    ...
    <>
      {programSelecionado.length == 0 ? (
        <main className="flex flex-1 flex-col  ">
          <Helmet>
            <title>Grupos de pesquisa | {'Simcc'}</title>
            <meta
              name="description"
              content={`Grupos de pesquisa | ${'Simcc'}`}
            />
                                  ...
                                  <AccordionContent>
                                    ...
                                    >
                                      {filteredTotal2.map((area) => (
                                        ...
                                        >
                                          <Alert
                                            className={` w-4 rounded-md border-0 h-4 p-0 ${qualisColor[normalizeArea(area || '')]}`}
                                          />{' '}
                                          {area}
                                        </ToggleGroupItem>
                                      ))}
          ...
          <div className="p-4 md:p-8">
            <div
              className={`${selectedAreas.length > 0 || selectedInstitutions.length > 0 ? 'flex' : 'hidden'} flex flex-wrap gap-3 mb-6 items-center`}
            >
              ...
              {selectedAreas.map((item) => (
                ...
                >
                  ...
                  <div
                    ...
                    onClick={() =>
                      handleAreaChange(selectedAreas.filter((i) => i !== item))
                    }
                  >
                    <X size={16} />
                  </div>
                  ...
                  <div
                    ...
                    onClick={() =>
                      handleInstitutionChange(
                        selectedInstitutions.filter((i) => i !== item),
                      )
                    }
                  >
                    <X size={16} />
