...
function PatentChartsAccordion({
  loading,
  chartData,
}: {
  loading: boolean;
  chartData: ChartMetricPatent[];
}) {
      ...
      <AccordionItem value="item-1">
        <div className="flex ">
          <HeaderResultTypeHome
            title="Gráfico de quantidade total de patentes"
...
export function PatentHome() {
  ...
  const totalPatents = useMemo(() => {
    return chartData.reduce(
      (acc, item) => acc + item.granted + item.not_granted,
      0,
    );
  }, [chartData]);
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      ...
      try {
        const response = await fetch(urlChartPatents, {
          ...fetchOptions,
