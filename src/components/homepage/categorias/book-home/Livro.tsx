...
export function BookItem(props: Publicacao) {
  ...
  const highlightedTitleEvent = highlightText(
    props.event_name || '',
    itemsSelecionados,
  );
  const highlightedTitle = highlightText(props.title || '', itemsSelecionados);
  const highlightedTitleProject = highlightText(
    props.project_name || '',
    itemsSelecionados,
  );
      ...
      <Alert className="rounded-l-none flex flex-col justify-between p-0 text-left">
        {props.has_image && (
          <div
            ...
            className="bg-neutral-100 flex justify-between p-4 bg-center bg-cover bg-no-repeat dark:bg-neutral-800 h-[150px] rounded-tr-md cursor-pointer"
            style={{ backgroundImage: `url(${urlGeral}image/${props.id}) ` }}
          >
            <div>
              {props.relevance && props.has_image && (
                <div className="relative -top-4 py-1 px-4 bg-yellow-600 w-fit rounded-b-md text-white">
                  <Star size={12} />
                </div>
              )}
            </div>
        ...
        <div className="p-4 pb-0">
          <div>
            ...
            {props.type == 'research-project' && (
              <p className="text-sm capitalize text-gray-500 dark:text-gray-300 font-normal">
                {highlightedTitleProject}
              </p>
            )}

            {props.type == 'patente' && (
              <h3 className="text-sm capitalize text-gray-500 dark:text-gray-300 font-normal ">
                {' '}
                {highlightedTitle}
              </h3>
            )}
            ...
            {props.type != 'patente' &&
              props.type != 'speaker' &&
              props.type != 'work-event' &&
              props.type != 'texto-revista' && (
                <p className="text-sm capitalize text-gray-500 dark:text-gray-300 font-normal">
                  {highlightedTitle}
                </p>
              )}
        ...
        <div className="flex items-center flex-wrap mt-4 gap-4 p-4 pt-0">
          <div className="flex items-center gap-4">
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
              ...
              {props.year}
              {props.type == 'research-project' &&
                `${props.start_year} - ${props.end_year == '' ? 'atual' : props.end_year}`}
              {props.year_}
            </div>

            {props.isbn != undefined && (
              <Link
                to={`https://www.cblservicos.org.br/isbn/pesquisa/?page=1&q=${props.isbn}&filtrar_por%5B0%5D=todos&ord%5B0%5D=relevancia&dir%5B0%5D=asc`}
                target="_blank"
                className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center"
              >
                <LinkBreak size={16} className="" />
                ISBN {props.isbn}
              </Link>
            )}
          </div>
          ...
          {(props.type == 'work-event' || props.type == 'texto-revista') && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
              <AppWindow size={12} />
              {props.means_divulgation?.split('_').join(' ')}
            </div>
          )}
          ...
          {props.type == 'research-project' && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
              <div
                className={`h-4 w-4 rounded-md ${props.status == 'EM_ANDAMENTO' ? 'bg-yellow-500' : props.status == 'DESATIVADO' ? 'bg-red-500' : 'bg-green-500'}`}
              ></div>
          ...
          {props.type == 'orientacoes' && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
              <Paperclip size={12} className={'whitespace-nowrap min-w-4'} />
              {props.nature}
            </div>
          )}
          ...
          {props.type == 'participacao-evento' && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
              <Ticket size={12} className={'whitespace-nowrap min-w-4'} />
              {props.nature}
            </div>
          )}

          {props.type == 'participacao-evento' &&
            props.participation != 'None' && (
              <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                <IdentificationBadge
                  size={12}
                  className={'whitespace-nowrap min-w-4'}
                />
                {props.participation}
              </div>
            )}
          ...
          {props.type == 'relatorio-tecnico' && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center  ">
              <CurrencyCircleDollar
                size={12}
                className=" w-3 whitespace-nowrap flex"
              />
              <p className="truncate max-w-[300px]">{props.financing}</p>
            </div>
          )}
          ...
          {props.type == 'orientacoes' && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-2 items-center">
              <div
                className={`w-4 h-4 rounded-md ${props.status == 'Em andamento' ? 'bg-yellow-500' : 'bg-green-500'}`}
              ></div>
              {props.status == 'Em andamento' ? 'Em andamento' : 'Concluída'}
            </div>
          )}
          ...
          {props.type == 'patente' && (
            <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-2 items-center">
              <div
                className={`w-4 h-4 rounded-md ${props.grant_date == 'NaT' || props.grant_date == 'None' || props.grant_date == '' || props.grant_date == null ? 'bg-red-500' : 'bg-green-500'}`}
              ></div>

              {props.grant_date == 'NaT' ||
              props.grant_date == 'None' ||
              props.grant_date == '' ||
              props.grant_date == null
                ? 'Sem concessão'
                : props.grant_date}
            </div>
          )}
          ...
          {!props.distinct && (
            <div
              ...
              onClick={() => onOpen2('researcher-modal', { name: props.name })}
            >
