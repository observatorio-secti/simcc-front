...
function ArticlesSummaryCard({
  publicacoesLength,
  percentage,
  distinct,
  setDistinct,
}: {
  publicacoesLength: number;
  percentage: number;
  distinct: boolean;
        ...
        <CardContent className="flex justify-between items-end">
          <div>
            <div className="text-2xl font-bold">
              {publicacoesLength.toLocaleString()}
            </div>
            <p className="text-xs text-muted-foreground flex gap-2">
              encontrados na busca{' '}
              <span className="text-eng-blue">
...
function ArticlesChartsAccordion({
  loading,
  publicacoes,
}: {
  loading: boolean;
...
function ArticlesListAccordion({
  loading,
  publicacoes,
  typeVisu,
  setTypeVisu,
...
export function ArticlesHome() {
  const { urlGeral, searchType, valoresSelecionadosExport } =
  ...
  return (
    <div className="grid grid-cols-1 gap-4 pb-16">
      ...
      <ArticlesChartsAccordion loading={loading} publicacoes={publicacoes} />
