...
export function EnviarNotificacoes() {
  ...
  const handleEnviarEmails = async () => {
    ...
    try {
      const response = await fetch(
        'https://simcc.uesc.br/v3/node/api/send-email',
        {
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        toast('Email enviado', {
          description:
            'Sua solicitação chegará na caixa de entrada de todos os usuários cadastrados',
          action: {
            label: 'Fechar',
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 p-4 md:p-8">
      <Helmet>
        <title>Enviar notificações | Módulo administrativo | {'Simcc'} </title>
        <meta
          name="description"
          content={`Enviar notificações | Módulo administrativo | ${'Simcc'}`}
        />
        ...
        <div className="flex lg:col-span-2 rounded-md border dark:border-neutral-800">
          <ScrollArea className="w-full">
            <div id="layout_email" className="w-full bg-neutral-50">
              <div
                className="bg-cover z-[0] bg-center bg-no-repeat h-64  rounded-t-md bg-eng-dark-blue w-full"
              ...
              <div className="max-w-[650px] mx-auto py-8 z-[2] relative -top-56">
                ...
                <div className="border bg-white border-neutral-200 rounded-b-md p-8">
                  <h1 className="font-medium text-3xl">
                    {titulo.length !== 0 ? titulo : 'Insira o título'}
                  </h1>
                  <p className="text-gray-500">
                    {subTitulo.length !== 0 ? subTitulo : 'Insira o subtítulo'}
                  </p>
                  <p className="py-12 flex relative text-gray-500">
                    {texto.length !== 0 ? texto : 'Insira o corpo da mensagem'}
                  </p>
                  {link.length > 0 && (
                ...
                <div className="pt-4">
                  <p className="text-gray-500 text-xs flex gap-1">
                    Você está recebendo este e-mail porque está inscrito na
                    Newsletter do{' '}
                    <a
                      href="https://conectee.eng.ufmg.br/"
                      target="_blank"
