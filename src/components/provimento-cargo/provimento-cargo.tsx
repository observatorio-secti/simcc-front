...
export function ProvimentoCargo() {
  ...
  const urlPatrimonio = `${urlGeral}ufmg/researcher?cpf=${cpf}&name=${search}`;
    ...
    <main className="flex flex-1 flex-col gap-4 md:gap-8  ">
      ...
      <div className="bg-cover bg-center bg-no-repeat">
        <div className="justify-center md:px-8 px-4 w-full mx-auto flex max-w-[980px] flex-col items-center gap-2 py-8 md:py-12 md:pb-8 lg:py-24 lg:pb-20">
          <Link
            to={'/informacoes'}
            className="inline-flex z-[2] items-center rounded-lg  bg-neutral-100 dark:bg-neutral-700  gap-2  px-3 py-1 text-sm font-medium"
          >
            <Info size={12} />
          ...
          <Alert className="h-14 mt-8 p-2 flex items-center justify-between lg:max-w-[60vw] lg:w-[60vw] w-full ">
            ...
            <div className="w-fit">
              <Button onClick={onClickBuscaPatrimonio} size={'icon'}>
                <Filter size={16} />
              </Button>
            </div>
          </Alert>
      ...
      {loading ? (
        <div>
          <Skeleton className="w-full rounded-md h-[300px]"></Skeleton>
        </div>
      ) : (
        info.length > 0 && (
          <div className="px-8">
