...
export function ConfirmDeleteResearcherGraduateProgram() {
  ...
  const isModalOpen =
    isOpen && typeModal === 'confirm-delete-researcher-graduate-program';
  ...
  const handleSubmitDelete = async () => {
    ...
    try {
      ...
      const urlProgram = urlGeralAdm + 'GraduateProgramResearcherRest/Delete';
      ...
      try {
        ...
        if (response.ok || response.status === 204) {
          ...
          toast('Tente novamente!', {
            description:
              'Erro ao remover pesquisador do programa de pós-graduação',
            action: {
              label: 'Fechar',
              onClick: () => {},
            },
      ...
      <DialogContent>
        <DialogHeader className="pt-8 px-6 flex flex-col items-center">
          <DialogTitle className="text-2xl  mb-2 font-medium max-w-[450px] text-center">
            <strong className="bg-red-500 text-white hover:bg-red-600 transition duration-500 font-medium px-1 py-0.5 rounded">
              Deletar
            </strong>{' '}
            pesquisador(a) {dataModal.name} do programa
          </DialogTitle>
          <DialogDescription className=" text-zinc-500 text-center">
            Você tem certeza de que deseja prosseguir com a exclusão do
            pesquisador que está atualmente vinculado a este programa de
            pós-graduação?
          </DialogDescription>
        </DialogHeader>
