...
export function ConfirmDeleteStudentGraduateProgram() {
  ...
  const isModalOpen =
    isOpen && typeModal === 'confirm-delete-student-graduate-program';
  const { urlGeralAdm } = useContext(UserContext);
  const [id_program, setIdProgram] = useState(
    dataModal && dataModal.graduate_program_id,
  );
  ...
  const handleSubmitDelete = async () => {
    try {
      ...
      const urlProgram = urlGeralAdm + '/studentRest/delete';
      ...
      <DialogContent>
        <DialogHeader className="pt-8 px-6 flex flex-col items-center">
          <DialogTitle className="text-2xl  mb-2 font-medium max-w-[450px]">
            ...
            discente {dataModal.nome} do programa
          </DialogTitle>
        ...
        <DialogFooter className=" py-4 ">
          <Button
            variant={'ghost'}
            onClick={() => onOpen('list-student-program')}
          >
