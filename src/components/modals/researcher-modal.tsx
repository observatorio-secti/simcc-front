...
export function ResearcherModal() {
  ...
  if (typeModal === 'researcher-modal') {
    urlTermPesquisadores =
      urlGeral +
      `researcherName?name=${name != null && name.split(' ').join(';')}`;
  }
  ...
  const [loadingMessage, setLoadingMessage] = useState(
    'Estamos procurando todas as informações do(a) pesquisador(a) no nosso banco de dados, aguarde.',
  );
    ...
    <>
      <Drawer open={isModalOpen} onClose={onClose}>
        <DrawerContent
          ...
          className={`max-h-[88%] pt-6 border border-b-0`}
        >
          {researcher.length === 0 && (
          ...
          {researcher.slice(0, 1).map((user) => {
            return (
              <div className="w-full flex justify-center ">
                <div
                  className="bg-cover bg-center bg-no-repeat h-28 w-28 bg-white dark:bg-neutral-950   rounded-2xl mb-3 border-4 border-white dark:border-neutral-950  absolute top-[-55px]   "
                  style={{
                  ...
                  <div className="xl:w-[350px] w-full grid grid-cols-1 ">
                    ...
                    >
                      <Masonry gutter="1px">
                        ...
                        {researcher.slice(0, 1).map((user) => {
                          return <Coautores id={user.id} name={user.name} />;
                        })}

                        {researcher.slice(0, 1).map(() => {
                          return (
                            <div>
                              <div className=" text-left mb-6 font-medium text-2xl">
                                Nomes de citação
                              </div>
