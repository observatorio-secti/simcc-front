...
export function ProjectModal() {
  ...
  return (
    <Sheet open={isModalOpen} onOpenChange={onClose}>
      ...
      >
        <div className={`h-full w-2 absolute bg-[#66B4D0]  `}></div>
              ...
              <div>
                ...
                <div className="flex items-center flex-wrap gap-4 mt-6">
                  <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                    ...
                    {data.start_year} -{' '}
                    {data.end_year == '' ? 'atual' : data.end_year}
                  </div>
                  <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                  ...
                  <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                    <div
                      className={`h-4 w-4 rounded-md ${data.status == 'EM_ANDAMENTO' ? 'bg-yellow-500' : data.status == 'DESATIVADO' ? 'bg-red-500' : 'bg-green-500'}`}
                    ></div>
            ...
            <div className="flex justify-between items-center flex-wrap gap-2">
              <div className="text-sm w-fit text-gray-500 dark:text-gray-300 font-normal flex gap-2 items-center">
                <Avatar className="cursor-pointer rounded-md  h-16 w-16">
                  <AvatarImage
                    ...
                    src={`${urlGeral}ResearcherData/Image?name=${data.researcher_name}`}
                  />
              ...
              <div className="flex justify-end w-full md:w-fit">
                <Link
                  ...
                  to={`/researcher?researcher_name=${data.researcher_name}&search_type=&terms=`}
                  target="_blank"
                >
                  <Button size={'icon'}>
            ...
            {data.production && data.production.length > 0 && (
              <div>
                ...
                <div className="flex flex-col gap-3">
                  {data.production.map((props, index) => (
                    <div className="flex">
                      <div
                        className={`w-2 min-w-2 rounded-l-md dark:border-neutral-800 border min-h-[120px] border-neutral-200 border-r-0 ${getQualisColor(props.type || '')} min-h-full relative`}
                      ></div>
                      <Alert key={index} className="rounded-l-none">
                        <div>
                  ...
                  {data.foment
                    ?.filter(
                      (item, index, self) =>
                        index ===
                        self.findIndex(
                          (t) => t.agency_name === item.agency_name,
                        ), // Filtra elementos únicos por 'name'
                    )
                    .map((props, index) => (
                      <div
                        key={index}
