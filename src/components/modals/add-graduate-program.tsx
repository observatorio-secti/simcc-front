...
import { useContext, useState, useEffect } from "react"; // Adicionado useEffect
...
function GraduateProgramAreaSelector({ value, onValueChange }: AreaSelectorProps) {
    return (
        <Select value={value} onValueChange={onValueChange}>
            <SelectTrigger id="area" className="items-start [&_[data-description]]:hidden">
                <SelectValue placeholder="Selecione a área" className={'whitespace-nowrap'} />
            </SelectTrigger>
            <SelectContent>
                <SelectGroup>
...
function GraduateProgramCitySelector({ selectedCity, onCitySelect }: CitySelectorProps) {
                ...
                <ScrollArea className="h-60 mt-2 border rounded-md">
                    {cities.length > 0 ? (
                        ...
                        <p className="p-2 text-sm text-gray-500">
                            {search.length < 3 ? "Digite 3+ letras" : "Nenhuma cidade encontrada"}
                        </p>
                    )}
                </ScrollArea>
            </PopoverContent>
...
function GraduateProgramHeader({ onClose }: HeaderProps) {
    return (
        <>
            ...
            <div className="p-8 pb-0"> {/* Movido P-8 para cá */}
                <div className="mb-8">
...
function GraduateProgramForm({ onClose }: FormProps) {
    ...
    return (
        <div className="p-8 pt-0"> {/* Restante do padding */}
            <div className="flex gap-4">
                <div className="flex flex-col gap-2 mt-4 w-2/3">
                    <Label>Nome do programa*</Label>
                    <Input value={name} onChange={(e) => setName(e.target.value)} type="text" disabled={isLoading} />
                </div>
                <div className="flex flex-col gap-2 mt-4 w-1/3">
                    <Label>Sigla</Label> {/* Removido * (não estava na validação) */}
                    <Input value={sigla} onChange={(e) => setSigla(e.target.value)} type="text" disabled={isLoading} />
                </div>
            </div>
            ...
            <div className="mt-4 gap-4 grid grid-cols-2">
                <div className="flex flex-col gap-2 w-full">
                    ...
                    <Select onValueChange={setModality} value={modality} disabled={isLoading}>
            ...
            <div className="mt-4 gap-4 flex">
                ...
                <div className="w-1/2 flex gap-4">
                    ...
                    <div className="flex flex-col gap-2 w-1/3">
                        ...
                        <Input value={ranking} onChange={(e) => setRanking(e.target.value)} type="text" disabled={isLoading} />
                    </div>
                </div>
            </div>
            ...
            <div className="flex gap-4 mt-4">
                <div className="flex flex-col gap-2  w-2/3">
                    ...
                    <Input value={code} onChange={(e) => setCode(e.target.value)} type="text" disabled={isLoading} />
                </div>
                <div className="flex flex-col gap-2 w-1/3">
                    <Label>Site</Label> {/* Removido * */}
                    <Input value={site} onChange={(e) => setSite(e.target.value)} type="text" disabled={isLoading} />
                </div>
            </div>
            ...
            <Button onClick={handleSubmit} disabled={isLoading} size={'sm'} className="text-white dark:text-white mt-3 ml-auto flex ">
