...
export function ConfirmDeletePosGraduateProgram() {
  ...
  const isModalOpen =
    isOpen && typeModal === 'confirm-delete-pos-graduate-program';
  const { urlGeralAdm } = useContext(UserContext);

  const id_delete = String(data.id_delete);
  const handleDeleteProgram = (id: string) => {
    const urlDeleteProgram =
      urlGeralAdm + `GraduateProgramRest/Delete?graduate_program_id=${id}`;
      ...
      <DialogContent>
        <DialogHeader className="pt-8 px-6 flex flex-col items-center">
          <DialogTitle className="text-2xl  mb-2 font-medium max-w-[450px]">
            ...
            pós-graduação em {data.name}
          </DialogTitle>
          <DialogDescription className=" text-zinc-500">
            É possível alterar informações básicas do programa. Caso delete,
        ...
        <DialogFooter className=" py-4 ">
          ...
          <Button
            variant={'destructive'}
            onClick={() => handleDeleteProgram(id_delete)}
          >
