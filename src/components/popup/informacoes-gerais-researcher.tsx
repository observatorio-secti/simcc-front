import { CalendarBlank, ChartLine, Quotes } from 'phosphor-react';
...
import { GraduationCap, Users, Blocks, Star, Building2 } from 'lucide-react';
...
function GraduateProgramCard({ program }: { program: GraduatePrograms }) {
  ...
  const [programDetails, setProgramDetails] =
    useState<GraduatePrograms>(program);
    ...
    const fetchDetails = async () => {
      try {
        const response = await fetch(
          `${urlGeral}graduate_program_profnit?id=${program.graduate_program_id}`,
          {
            mode: 'cors',
            headers: {
        ...
        if (data && data.length > 0) {
          ...
          if (full.institution_id) {
            const institutionRes = await fetch(
              `${urlGeral}institution/${full.institution_id}/`,
            );
  ...
  return (
    <div className="h-fit text-left w-full">
      ...
      <div className="flex gap-3 mb-6 items-center flex-wrap">
        ...
        >
          <CalendarBlank size={12} /> Atualização do Lattes:{' '}
          {String(props.data_atualizacao_lattes)}
        </div>
      </div>
