    ...
    cell: ({ row }) => {
      return (
        <div className="flex items-center gap-4">
          {row.original.functional_classification}{' '}
          {row.original.other_functional_classification != null &&
            `- ${row.original.other_functional_classification}`}
        </div>
      );
    },
  },
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center whitespace-nowrap">
          <CalendarBlank size={12} /> {row.original.start_year} -{' '}
          {row.original.end_year == null || row.original.end_year == ''
            ? 'Atual'
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center">
          {' '}
          <Clock size={12} /> {row.original.workload_hours_weekly} horas
        </div>
      );
    },
  },

  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center whitespace-nowrap">
          {' '}
          <TextSelectionIcon size={12} />{' '}
          {row.original.employment_type.split('_').join(' ')}{' '}
        </div>
      );
    },
  },

  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center">
          {' '}
          {row.original.exclusive_dedication ? (
            <Check size={12} />
          ) : (
            <X size={12} />
          )}{' '}
          {row.original.exclusive_dedication ? 'Sim' : 'Não'}{' '}
        </div>
      );
    },
  },
