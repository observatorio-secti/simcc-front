...
export const columnsProjetoPesquisa: ColumnDef<ProjetoPesquisa>[] = [
    ...
    cell: ({ row }) => {
      return (
        <div className="flex items-center gap-4">
          {' '}
          {row.getValue('nature') != undefined && row.getValue('nature')}
        </div>
      );
    },
  },
      ...
      return (
        <div className="flex gap-1 items-center">
          ...
          <div className="whitespace-nowrap">
            ...
            {row.original.end_year == '' ? 'atual' : row.original.end_year}
          </div>
        </div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="text-sm text-gray-500 whitespace-nowrap dark:text-gray-300 font-normal flex gap-1 items-center">
          <div
            className={`h-4 w-4 rounded-md ${row.original.status == 'EM ANDAMENTO' ? 'bg-yellow-500' : row.original.status == 'DESATIVADO' ? 'bg-red-500' : 'bg-green-500'}`}
          ></div>
