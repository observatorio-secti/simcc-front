  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex gap-1 items-center">
          <div
            className={`w-4 h-4 rounded-md  ${qualisColor[row.getValue('qualis') as keyof typeof qualisColor]}`}
          ></div>{' '}
          {row.getValue('qualis')}
        </div>
      );
      ...
      return (
        <div>
          {jif != 'None' && jif.length != 0 && (
            <a
              href={row.getValue('jcr_link')}
              target="_blank"
              rel="noopener noreferrer"
              className="border-[1px] cursor-pointer border-gray-300 py-2 flex px-4 text-gray-400 rounded-md text-xs font-medium gap-2 items-center  whitespace-nowrap"
            >
              <LinkBreak size={16} className="text-gray-400" />
              JCR {jif}
            </a>
          )}
        </div>
