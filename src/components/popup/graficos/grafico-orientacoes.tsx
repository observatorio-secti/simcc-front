...
export function GraficoOrientacoes(props: { chartData: ChartMetricGuidance[] }) {
    ...
    if (props.chartData) {
      const formattedData = props.chartData.map(item => {
        ...
        return {
          ...
          "monografia de conclusao de curso aperfeicoamento e especializacao": e,
  ...
  return (
    <Alert className="pt-12">
      <ChartContainer config={normalizedChartConfig} className="w-full h-[250px]">
        <ResponsiveContainer>
          <BarChart data={chartDataFormatted} margin={{ top: 20, right: 20, left: 20, bottom: 0 }}>
