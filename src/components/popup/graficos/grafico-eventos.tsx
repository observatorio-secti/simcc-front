...
export function GraficosEventos({ chartData }: { chartData: ChartMetricEvent[] }) {
  // Apenas formata o array somando os totais para o LabelList 
  const formattedData = chartData.map(item => ({
    ...item,
    year: String(item.year),
    total: item.congress + item.meeting + item.workshop + item.other + item.seminar + item.symposium
  }));
  ...
  return (
    <Alert className="pt-12">
      <ChartContainer config={chartConfig} className="h-[250px] w-full">
        <ResponsiveContainer>
          <BarChart data={formattedData} margin={{ top: 20, right: 0, left: 0, bottom: 0 }}>
