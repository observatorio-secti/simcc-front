...
export function RelevanceProduction(props: Props) {
  ...
  const urlTermPublicacoes = useMemo(() => {
    ...
    return searchType === 'article' ? `${baseUrl}&terms=` : baseUrl;
  }, [props.name]);
  ...
  const urlTermBook = `${urlGeral}book_production_researcher?researcher_id=${props.name}&year=1900&term=`;
  ...
  const urlTermCap = `${urlGeral}book_chapter_production_researcher?researcher_id=${props.name}&year=1900&term=`;
  ...
  const urlTermPatent = `${urlGeral}patent_production_researcher?researcher_id=${props.name}&year=1900&term=`;
