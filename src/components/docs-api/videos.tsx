...
export function Videos() {
  ...
  function parseVideoInfo(filename: string) {
    ...
    const titleMap: Record<string, string> = {
      ...
      'Índice do Pesquisador ‐ Feito com o Clipchamp (1).mp4':
        'Índice Do Pesquisador',
      'Bolsista de Produtividade_new ‐ Feito com o Clipchamp.mp4':
    ...
    const title =
      titleMap[filename] ||
      filename.replace(/ ‐ Feito com o Clipchamp.*|\.mp4$/gi, '');
    ...
    if (filename === 'power-bi-tutorial') {
      ...
      if (title.toLowerCase().includes('bolsista'))
        description = `Saiba como identificar e analisar bolsistas de produtividade na plataforma ${platform}.`;
      if (title.toLowerCase().includes('qualis'))
        description = `Explicação sobre o Qualis e sua aplicação na avaliação de periódicos e produção acadêmica na plataforma ${platform}.`;
      if (title.toLowerCase().includes('orcid'))
        description = `Descubra o que é o ORCID, para que serve e como ele pode ser usado para reunir e autenticar sua produção científica na plataforma ${platform}.`;
      if (title.toLowerCase().includes('jcr'))
        description = `Entenda o Journal Citation Reports (JCR) e como ele é utilizado para avaliar o impacto de periódicos científicos.`;
      if (title.toLowerCase().includes('índice h'))
        description = `Aprenda a calcular e interpretar o Índice H para medir a produtividade e o impacto de pesquisadores.`;
      if (title.toLowerCase().includes('doi'))
        description = `O que é DOI, como encontrar e utilizar o identificador digital de objetos em publicações científicas.`;
    }
  ...
  const videos = videoFiles.map((file, idx) => {
    ...
    return {
      src:
        file === 'power-bi-tutorial'
          ? 'https://www.youtube.com/embed/8HI3rnNhTNo'
          : `/vid/${file}`,
      title,
      description,
      id: idx,
  ...
  return (
    <main className="p-4 md:p-8 bg-neutral-50 dark:bg-neutral-900 text-gray-800 dark:text-gray-100">
      <Helmet>
        ...
        <meta
          name="description"
          content={`Tutoriais em vídeo sobre a plataforma ${platform}`}
        />
      ...
      <div className="max-w-[1200px] mx-auto space-y-8">
        ...
        <Alert className="space-y-4 p-8">
          ...
          <p className="text-base text-justify">
            Nesta seção você encontra vídeos explicativos sobre os principais
            recursos, índices e funcionalidades da plataforma {platform}.
            Assista para aprender a utilizar indicadores, entender conceitos e
        ...
        <div className="flex flex-col gap-8">
          {videos.map((video, idx) => (
            <Alert
              key={video.id}
              className="flex flex-col md:flex-row items-stretch rounded-md overflow-hidden bg-white dark:bg-neutral-900 p-0 border border-gray-200 dark:border-neutral-800"
            >
              {/* Vídeo à esquerda */}
              ...
              <div className="flex flex-col justify-center items-center md:items-start text-center md:text-left p-6 md:w-1/2 w-full font-lexend">
                ...
                <p className="text-base text-muted-foreground text-justify md:text-left mx-auto md:mx-0 max-w-prose">
                  {video.description}
                </p>
              </div>
            </Alert>
