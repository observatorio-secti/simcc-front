...
export function DisciplinasDepartamentoPage(props: Props) {
  ...
  const [dataSelecionado, setDataSelecionado] = useState<Disciplinas | null>(
    null,
  );

  const { urlGeralAdm, urlGeral } = useContext(UserContext);
  const urlDisciplinas =
    urlGeralAdm + `departamentos/disciplinas?dep_id=${props.dep_id}`;
  ...
  const filteredTotal = Array.isArray(data)
    ? data.filter((item) => {
        ...
        const matchesCourse2 =
          selectedDiscipline2 === '' ||
          item.academic_activity_name === selectedDiscipline2;

        // Filtro por demanding_courses
        const matchesCourse =
          selectedDiscipline === '' ||
          item.demanding_courses
            .split(',')
        ...
        const matchesProfessor =
          selectedProfessor === '' ||
          item.researcher_name.includes(selectedProfessor);

        return (
          matchesName && matchesCourse2 && matchesCourse && matchesProfessor
        );
      })
    : [];
  ...
  const combineItems = (items: Disciplinas[]) => {
    ...
    items.forEach((item) => {
      ...
      combinedItems[key].occupied_slots = String(
        Number(combinedItems[key].occupied_slots) + Number(item.occupied_slots),
      );
      combinedItems[key].percent_occupied_slots =
        (Number(combinedItems[key].occupied_slots) /
          combinedItems[key].available_slots) *
        100;
      combinedItems[key].demanding_courses =
        `${combinedItems[key].demanding_courses},${item.demanding_courses}`;
      combinedItems[key].turmas_juntas = combinedItems[key].turmas_juntas
        ? `true`
        : ''; // Atualiza o campo turmas_juntas
    });
  ...
  const uniqueDisciplines = [
    ...new Set(
      filteredTotal
        .map((d) => d.academic_activity_name)
  ...
  const uniqueCursos = [
    ...new Set(
      data
        .flatMap((d) =>
          d.demanding_courses.split(',').map((discipline) => discipline.trim()),
        )
        .filter((name) => name.length !== 0),
    ),
  ];
  ...
  return (
    <main className="grid flex-1 items-start gap-4  md:gap-8 lg:grid-cols-3 xl:grid-cols-3 px-4 md:px-8 pb-4 md:pb-8">
      <div
        className={`grid auto-rows-max items-start gap-4 md:gap-8 ${dataSelecionado ? 'lg:col-span-2' : 'lg:col-span-3'}`}
      >
        <div className="grid gap-4 mt-8 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
          <Alert
            className="p-0 bg-cover bg-no-repeat bg-center lg:col-span-2"
          ...
          >
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {combinedFilteredTotalLenght.length}
              </div>
              <div className="flex items-center gap-3">
                <p className="text-xs text-muted-foreground">do departamento</p>
              </div>
            </CardContent>
          </Alert>
        ...
        <div className="flex gap-3 items-center w-full rounded-md border border-neutral-200 bg-white px-3 py-2 text-sm file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-neutral-500 focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50 dark:border-neutral-800 dark:bg-neutral-950 dark:placeholder:text-neutral-400">
          ...
          <div className="flex items-center gap-3">
            ...
            <Select
              value={selectedDiscipline2}
              onValueChange={setSelectedDiscipline2}
            >
              <SelectTrigger className="">
          ...
          <Masonry gutter="16px">
            {combinedFilteredTotal.slice(0, count).map((props, index) => {
              const uniqueResearchers = [
                ...new Map(
                  props.researcher_name.map((item) => [item, item]),
                ).values(),
              ];

              return (
                <div
                  onClick={() => setDataSelecionado(props)}
                  className="flex  w-full cursor-pointer"
                  key={index}
                >
                  <div
                    className={`h-full w-2 rounded-l-md dark:border-neutral-800 border border-neutral-200 border-r-0 ${
                      props.occupied_slots === '' ||
                      props.occupied_slots === '0'
                        ? 'bg-red-500'
                        : 'bg-green-500'
                    }`}
                  ></div>

                  <Alert className="p-0 group rounded-l-none h-auto">
                    <CardHeader className="flex p-4 flex-row items-start bg-neutral-100 dark:bg-neutral-800 rounded-tr-md">
                      <div className="grid gap-0.5">
                        <CardTitle className="group flex items-center w-fit gap-2 text-base">
                          <div className="w-fit">
                            {props.academic_activity_name}
                          </div>
                        </CardTitle>
                        <div className="flex gap-3 flex-wrap">
                          <CardDescription className="flex gap-1 items-center">
                            <Hash size={12} />
                            {props.academic_activity_code}
                          </CardDescription>
                          <CardDescription className="flex gap-1 items-center">
                            <Clock size={12} />
                            {props.academic_activity_ch}h
                          </CardDescription>
                          <CardDescription className="flex gap-1 items-center">
                            <Clock size={12} />
                            {props.status}
                      ...
                      <div className="flex gap-2 items-center">
                        {uniqueResearchers
                          ...
                          .map((item, index) => (
                            <div
                              key={index}
                              className={`flex items-center ${props.researcher_id[index] && 'cursor-pointer'} cursor-pointer justify-between`}
                            >
