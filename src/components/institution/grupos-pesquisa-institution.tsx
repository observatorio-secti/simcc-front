import { Blocks, Building2, ChevronDown, ChevronUp, Download, File, Plus, Shapes, SlidersHorizontal, Trash, Users, X } from "lucide-react";
...
export function GruposPesquisaInstitution({ institutionId, institutionName }: GruposPesquisaInstitutionProps) {
                  ...
                  <DropdownMenuContent className="w-56">
                    ...
                    <ScrollArea className="h-72">
                      {filteredTotal2.map((area) => (
                        ...
                        >
                          <Alert className={`w-4 rounded-md border-0 h-4 p-0 mr-2 ${qualisColor[normalizeArea(area || '')]}`} />
                          {area}
                        </DropdownMenuCheckboxItem>
                      ))}
                    </ScrollArea>
                        ...
                        {filteredTotal.slice(0, count).map((item) => {
                          const handlePesquisaFinal = (id: string) => {
                            ...
                            if (institutionId && institutionName) {
                              ...
                              queryParams.set('institution_name', institutionName);
                            }
                            
                            navigate(`/grupos-pesquisa?${queryParams.toString()}`);
                          }

                          return (
                            <div key={item.id} className="flex w-full" onClick={() => handlePesquisaFinal(item.id)}>
                              ...
                              >
                                <div className="flex w-full flex-col gap-1">
                                  <div className="flex justify-between items-center">
                                    <div className="text-xs font-medium mb-2 flex items-center gap-2">
                                      {item.area != '' ? (item.area) : ('Sem código')}
                                    </div>
                                    <Shapes size={16} />
                                  </div>
                                ...
                                <div className="line-clamp-2 flex-wrap text-xs text-muted-foreground flex gap-3">
                                  <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                                    <Users size={12} />{item.first_leader}
                                  </div>
                                  <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                                  ...
                                  {(item.second_leader != '' && item.second_leader != null) && (
                                    <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                                      <Users size={12} />{item.second_leader}
                                    </div>
                                  )}
                                </div>
                              </button>
