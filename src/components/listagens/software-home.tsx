...
export type ChartMetricSoftware = {
    year: number
    among: number
}

function SoftwareSummaryCard({ totalSoftware, distinct, setDistinct }: { totalSoftware: number, distinct: boolean, setDistinct: (val: boolean) => void }) {
    return (
        <div className="mt-4">
            <Alert className={`p-0 bg-cover bg-no-repeat bg-center `}>
                ...
                <CardContent className="flex justify-between items-end">
                    <div>
                        <div className="text-2xl font-bold">{totalSoftware.toLocaleString()}</div>
...
function SoftwareChartsAccordion({ loading, chartData }: { loading: boolean, chartData: ChartMetricSoftware[] }) {
            ...
            <AccordionItem value="item-1">
                ...
                <AccordionContent className="p-0">
                    {loading ? (
                        ...
                        <GraficoSoftware software={publicacoesFormatted} />
                    )}
                </AccordionContent>
            </AccordionItem>
        </Accordion>
...
function SoftwareListAccordion({ loading, publicacoes, typeVisu, setTypeVisu, distinct, onLoadMore, hasMore }: { loading: boolean, publicacoes: Patente[], typeVisu: string, setTypeVisu: (val: string) => void, distinct: boolean, onLoadMore: () => void, hasMore: boolean }) {
...
export function SoftwareHome() {
    ...
    const [chartData, setChartData] = useState<ChartMetricSoftware[]>([]);
    ...
    const urlTermPublicacoes = useMemo(() => {
        ...
        return `${urlGeral}software_production_researcher?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}${paginationParams}`;
    }, [urlGeral, yearString, distinct, institutionParam, page]);

    const urlChartSoftware = useMemo(() => {
        return `${urlGeral}metrics/software/chart?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}`;
    ...
    useEffect(() => {
        ...
        const fetchChartData = async () => {
            ...
            try {
                const response = await fetch(urlChartSoftware, { ...fetchOptions, signal: abortController.signal });
    ...
    return (
        <div className="grid grid-cols-1 gap-4 pb-16">
            ...
            <SoftwareChartsAccordion
                loading={loadingCharts}
                chartData={chartData}
            />

            <SoftwareListAccordion
                loading={loading}
