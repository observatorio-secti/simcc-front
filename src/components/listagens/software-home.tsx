...
function SoftwareChartsAccordion({
  loading,
  chartData,
}: {
  loading: boolean;
  chartData: ChartMetricSoftware[];
}) {
  const publicacoesFormatted = useMemo(() => {
    return chartData.flatMap((item) =>
      Array.from(
        { length: item.among },
        () => ({ year: String(item.year) }) as Patente,
      ),
    );
  }, [chartData]);
      ...
      <AccordionItem value="item-1">
        <div className="flex ">
          <HeaderResultTypeHome
            title="Gráfico de quantidade total de softwares"
...
function SoftwareListAccordion({
  loading,
  publicacoes,
  typeVisu,
  setTypeVisu,
...
export function SoftwareHome() {
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      ...
      try {
        const response = await fetch(urlChartSoftware, {
          ...fetchOptions,
