...
function ReportChartsAccordion({
  loading,
  chartData,
}: {
  loading: boolean;
  chartData: ChartMetricReport[];
}) {
      ...
      <AccordionItem value="item-1">
        <div className="flex ">
          <HeaderResultTypeHome
            title="Gráfico de quantidade total de relatórios técnicos"
...
export function RelatorioTecnicoHome() {
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      ...
      try {
        const response = await fetch(urlChartReport, {
          ...fetchOptions,
