...
import { Info, MapIcon, Trash, User, X } from 'lucide-react';
    ...
    if (selectedAreas.length > 0) {
      filtered = filtered.filter((r) =>
        selectedAreas.some((selectedArea) =>
          (r.area ? r.area.split(';').map((area) => area.trim()) : []).some(
            (area) => area.includes(selectedArea),
          ),
        ),
      );
    }
...
export function ResearchersHomeListagens() {
  ...
  const urlMetrics = `${urlGeral}researcher_metrics`;
                    ...
                    <Alert className="">
                      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                        <div>
                          ...
                          <CardDescription>
                            Tempo de atualização desde {String(currentDate)}
                          </CardDescription>
                        </div>
