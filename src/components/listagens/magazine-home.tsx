...
function MagazineSearchBar({ pesquisaInput, setPesquisaInput }: { pesquisaInput: string, setPesquisaInput: (val: string) => void }) {
    return (
    ...
    return (
        <div className="mt-6">
            <Alert className="p-0 bg-cover bg-no-repeat bg-center">
                ...
                <CardContent>
                    <div className="text-2xl font-bold">{totalMagazines.toLocaleString()}</div>
...
function MagazineListAccordion({ loading, publicacoes, typeVisu, setTypeVisu, onLoadMore, hasMore }: { loading: boolean, publicacoes: Revista[], typeVisu: string, setTypeVisu: (val: string) => void, onLoadMore: () => void, hasMore: boolean }) {
                                        ...
                                        <Masonry gutter="16px">
                                            {publicacoes.map((props, index) => (
                                                <div className="flex w-full" key={props.id || index}>
                                                    <div className={`h-full w-2 min-w-[8px] rounded-l-lg border border-r-0 border-neutral-200 dark:border-neutral-800 ${qualisColor[props.qualis as keyof typeof qualisColor] || qualisColor['None']}`}></div>
                                                            ...
                                                            <div>
                                                                ...
                                                                <div className="mt-2 flex items-center gap-4">
                                                                    <div className="text-sm text-gray-500 dark:text-gray-300 font-normal flex gap-1 items-center">
                                                                        <div className={`w-4 h-4 rounded-md ${qualisColor[props.qualis as keyof typeof qualisColor] || qualisColor['None']}`}></div>Qualis {props.qualis}
                                                                    </div>
...
export function MagazineHome() {
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(urlMagazine, { ...fetchOptions, signal: abortController.signal });
    ...
    useEffect(() => {
        ...
        const fetchMetrics = async () => {
            try {
                const response = await fetch(urlMetrics, { ...fetchOptions, signal: abortController.signal });
                const data = await response.json();
