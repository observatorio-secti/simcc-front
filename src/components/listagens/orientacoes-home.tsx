...
import { ChartBar, Rows, SquaresFour, Student } from "phosphor-react"
...
function GuidanceSummaryCard({ totalGuidance, distinct, setDistinct }: { totalGuidance: number, distinct: boolean, setDistinct: (val: boolean) => void }) {
    return (
        <div className="mt-4">
            <Alert className={`p-0 bg-cover bg-no-repeat bg-center `}>
                ...
                <CardContent className="flex justify-between items-end">
                    <div>
                        <div className="text-2xl font-bold">{totalGuidance.toLocaleString()}</div>
...
function GuidanceChartsAccordion({ loading, chartData }: { loading: boolean, chartData: ChartMetricGuidance[] }) {
    return (
            ...
            <AccordionItem value="item-1">
                ...
                <AccordionContent className="p-0">
                    {loading ? (
                        ...
                        <GraficoOrientacoes chartData={chartData} />
                    )}
                </AccordionContent>
            </AccordionItem>
        </Accordion>
...
function GuidanceListAccordion({ loading, publicacoes, typeVisu, setTypeVisu, distinct, onLoadMore, hasMore }: { loading: boolean, publicacoes: Patente[], typeVisu: string, setTypeVisu: (val: string) => void, distinct: boolean, onLoadMore: () => void, hasMore: boolean }) {
...
export function OrientacoesHome() {
    ...
    const [chartData, setChartData] = useState<ChartMetricGuidance[]>([]);
    ...
    const totalGuidance = useMemo(() => {
        return chartData.reduce((acc, item) => {
            return acc +
                item.m_completed + item.m_in_progress +
                item.ic_completed + item.ic_in_progress +
                item.d_completed + item.d_in_progress +
                item.g_completed + item.g_in_progress +
                item.e_completed + item.e_in_progress +
                item.sd_completed + item.sd_in_progress;
        }, 0);
    }, [chartData]);

    const urlTermPublicacoes = useMemo(() => {
        const paginationParams = `&page=${page}&lenght=${limit}&sort_by=year&sort_order=desc`;
        return `${urlGeral}guidance_researcher?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}${paginationParams}`;
    }, [urlGeral, yearString, distinct, institutionParam, page]);

    const urlChartGuidance = useMemo(() => {
        return `${urlGeral}metrics/guidance/chart?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}`;
    ...
    useEffect(() => {
        ...
        const fetchChartData = async () => {
            ...
            try {
                const response = await fetch(urlChartGuidance, { ...fetchOptions, signal: abortController.signal });
    ...
    return (
        <div className="grid grid-cols-1 gap-4 pb-16">
            ...
            <GuidanceSummaryCard
                totalGuidance={totalGuidance}
                distinct={distinct}
                setDistinct={(val) => { setDistinct(val); setPage(1); }}
            />

            <GuidanceChartsAccordion
                loading={loadingCharts}
                chartData={chartData}
            />

            <GuidanceListAccordion
                loading={loading}
