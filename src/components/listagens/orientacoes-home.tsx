...
function GuidanceChartsAccordion({
  loading,
  chartData,
}: {
  loading: boolean;
      ...
      <AccordionItem value="item-1">
        <div className="flex ">
          <HeaderResultTypeHome
            title="Gráfico de quantidade total de orientações"
...
function GuidanceListAccordion({
  loading,
  publicacoes,
  typeVisu,
  setTypeVisu,
...
export function OrientacoesHome() {
  ...
  useEffect(() => {
    ...
    const fetchChartData = async () => {
      ...
      try {
        const response = await fetch(urlChartGuidance, {
          ...fetchOptions,
