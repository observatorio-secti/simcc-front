...
export type ChartMetricWorkEvent = {
    year?: number
    among: number
}

function WorkEventSummaryCard({ totalWorkEvents, distinct, setDistinct }: { totalWorkEvents: number, distinct: boolean, setDistinct: (val: boolean) => void }) {
    return (
        <div className="mt-4 ">
            <Alert className={`p-0 bg-cover bg-no-repeat bg-center `}>
                ...
                <CardContent className="flex justify-between items-end">
                    <div>
                        <div className="text-2xl font-bold">{totalWorkEvents.toLocaleString()}</div>
...
function WorkEventChartsAccordion({ loading, chartData }: { loading: boolean, chartData: ChartMetricWorkEvent[] }) {
    // Expande os dados agregados para o array simulado esperado pelo GraficoTrabalhoEvento
    const publicacoesFormatted = useMemo(() => {
        return chartData.flatMap(item =>
            Array.from({ length: item.among }, () => ({ year_: String(item.year || '') } as Patente))
        );
    }, [chartData]);
            ...
            <AccordionItem value="item-1" >
                ...
                <AccordionContent className="p-0">
                    {loading ? (
                        ...
                        <GraficoTrabalhoEvento publicacoes={publicacoesFormatted} />
                    )}
                </AccordionContent>
            </AccordionItem>
        </Accordion>
...
function WorkEventListAccordion({ loading, publicacoes, typeVisu, setTypeVisu, distinct, onLoadMore, hasMore }: { loading: boolean, publicacoes: Patente[], typeVisu: string, setTypeVisu: (val: string) => void, distinct: boolean, onLoadMore: () => void, hasMore: boolean }) {
...
export function WorkEventHome() {
    ...
    const [chartData, setChartData] = useState<ChartMetricWorkEvent[]>([]);
    ...
    const urlTermPublicacoes = useMemo(() => {
        ...
        return `${urlGeral}researcher_production/events?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}${paginationParams}`;
    }, [urlGeral, yearString, distinct, institutionParam, page]);

    const urlChartEvents = useMemo(() => {
        return `${urlGeral}metrics/events/chart?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}`;
    ...
    return (
        <div className="grid grid-cols-1 gap-4 pb-16 ">
            ...
            <WorkEventSummaryCard
                totalWorkEvents={totalWorkEvents}
                distinct={distinct}
                setDistinct={(val) => { setDistinct(val); setPage(1); }}
            />

            <WorkEventChartsAccordion
                loading={loadingCharts}
                chartData={chartData}
            />

            <WorkEventListAccordion
                loading={loading}
