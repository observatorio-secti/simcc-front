...
function PapersMagazineSummaryCard({ totalPapers, distinct, setDistinct }: { totalPapers: number, distinct: boolean, setDistinct: (val: boolean) => void }) {
    return (
        <div className="mt-4 ">
            <Alert className={`p-0 bg-cover bg-no-repeat bg-center `}>
                ...
                <CardContent className="flex justify-between items-end">
                    <div>
                        <div className="text-2xl font-bold">{totalPapers.toLocaleString()}</div>
...
function PapersMagazineChartsAccordion({ loading, chartData }: { loading: boolean, chartData: ChartMetricPapersMagazine[] }) {
    ...
    return (
        <Accordion type="single" collapsible defaultValue="item-1">
            <AccordionItem value="item-1" >
                <div className="flex ">
                    <HeaderResultTypeHome title="Gráfico de quantidade total de textos em revista" icon={<ChartBar size={24} className="text-gray-400" />} />
                    <AccordionTrigger />
                </div>
...
function PapersMagazineListAccordion({ loading, publicacoes, typeVisu, setTypeVisu, distinct, onLoadMore, hasMore }: { loading: boolean, publicacoes: Patente[], typeVisu: string, setTypeVisu: (val: string) => void, distinct: boolean, onLoadMore: () => void, hasMore: boolean }) {
...
export function TextoRevistaHome() {
    ...
    useEffect(() => {
        ...
        const fetchChartData = async () => {
            ...
            try {
                const response = await fetch(urlChartPapersMagazine, { ...fetchOptions, signal: abortController.signal });
