...
export type ChartMetricBrand = {
    year: number
    among: number
}

function BrandSummaryCard({ totalBrands, distinct, setDistinct }: { totalBrands: number, distinct: boolean, setDistinct: (val: boolean) => void }) {
    return (
        <div className="mt-4">
...
function BrandChartsAccordion({ loading, chartData }: { loading: boolean, chartData: ChartMetricBrand[] }) {
            ...
            <AccordionItem value="item-1">
                ...
                <AccordionContent className="p-0">
                    {loading ? (
                        ...
                        <GraficoMarca marca={publicacoesFormatted} />
                    )}
                </AccordionContent>
            </AccordionItem>
        </Accordion>
...
function BrandListAccordion({ loading, publicacoes, typeVisu, setTypeVisu, distinct, onLoadMore, hasMore }: { loading: boolean, publicacoes: Patente[], typeVisu: string, setTypeVisu: (val: string) => void, distinct: boolean, onLoadMore: () => void, hasMore: boolean }) {
...
export function BrandHome() {
    ...
    const [chartData, setChartData] = useState<ChartMetricBrand[]>([]);
    ...
    const urlTermPublicacoes = useMemo(() => {
        ...
        return `${urlGeral}brand_production_researcher?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}${paginationParams}`;
    }, [urlGeral, yearString, distinct, institutionParam, page]);

    const urlChartBrand = useMemo(() => {
        return `${urlGeral}metrics/brand/chart?researcher_id=&year=${yearString}&distinct=${distinct ? '1' : '0'}${institutionParam}`;
    ...
    useEffect(() => {
        ...
        const fetchChartData = async () => {
            ...
            try {
                const response = await fetch(urlChartBrand, { ...fetchOptions, signal: abortController.signal });
