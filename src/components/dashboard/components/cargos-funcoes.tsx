...
export function CargosFuncoes() {
  ...
  const urlDisciplinas = urlGeralAdm + `s/role`;
  ...
  const handleSubmitPesquisador = async () => {
    try {
      ...
      const urlProgram = urlGeralAdm + 's/role';
        ...
        try {
          ...
          if (response.ok) {
            toast('Dados enviados com sucesso', {
              description:
                'Cargo adicionado a coleção, adicione as funcionalidades',
              action: {
                label: 'Fechar',
                onClick: () => {},
              },
    ...
    <Tabs defaultValue={tab} value={tab}>
      ...
      >
        ...
        <div className={`flex pt-2 justify-between  ${isOn ? '' : ''} `}>
          <div className="flex items-center gap-2">
            <div
              className={`pb-2 border-b-2 transition-all ${tab == 'cargos' ? 'border-b-[#719CB8]' : ' border-b-transparent '}`}
            >
              <Button
                disabled={!has_editar_cargos_permissoes}
                variant={tab == 'cargos' ? 'ghost' : 'ghost'}
            ...
            <div
              className={`pb-2 border-b-2 transition-all ${tab == 'usuarios' ? 'border-b-[#719CB8]' : ' border-b-transparent '}`}
            >
              <Button
                disabled={!has_editar_cargos_usuarios}
                variant={tab == 'usuarios' ? 'ghost' : 'ghost'}
            ...
            <div
              className={`pb-2 border-b-2 transition-all ${tab == 'pesquisadores' ? 'border-b-[#719CB8]' : ' border-b-transparent '}`}
            >
              <Button
                disabled={!has_editar_informacoes_usuarios}
                variant={tab == 'pesquisadores' ? 'ghost' : 'ghost'}
                onClick={() => setTab('pesquisadores')}
              >
      ...
      <TabsContent value="cargos">
        <main className="grid flex-1 items-start gap-4 p-4 md:p-8 md:gap-8 lg:grid-cols-3 xl:grid-cols-3  pb-4 md:pb-8">
          ...
          >
            <Alert className="p-0" x-chunk="dashboard-05-chunk-0">
              <CardHeader className="pb-3">
                <CardTitle>Adicionar cargo</CardTitle>
                <CardDescription className="max-w-lg text-balance leading-relaxed">
                  Adicione o cargo, este poderá ser atrelado a um docente ou
                  técnico, possuindo acesso a funcionalidades da plataforma.
                </CardDescription>
              </CardHeader>
              <CardFooter>
              ...
              <Masonry gutter="16px">
                {isLoad
                  ? Array.from({ length: 5 }).map((_, index) => (
                      <Skeleton
                        key={index}
                        className="w-full h-[54px] rounded-md"
                      />
                    ))
                  : data.map((props) => (
          ...
          {dataSelecionado && (
            <DisplayCargo role={dataSelecionado.role} id={dataSelecionado.id} />
          )}
        </main>
      </TabsContent>
