...
export function GraficoDisciplinas({
  disciplinas,
}: {
  ...
  useEffect(() => {
    ...
    const filtered = disciplinas.filter(
      (d) =>
        (selectedDiscipline
          ? d.academic_activity_name === selectedDiscipline
          : true) &&
    ...
    const aggregatedData = Object.keys(professorHours).map((semester) => ({
      semester,
      hours: Object.values(professorHours[semester]).reduce(
        (sum, hours) => sum + hours,
        0,
      ),
    }));
  ...
  const uniqueDisciplines = [
    ...new Set(disciplinas.map((d) => d.academic_activity_name)),
  ];
