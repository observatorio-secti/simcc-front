          ...
          {filteredTotal.slice(0, count).map((item) => {
            ...
            return (
              <div className="flex" onClick={() => updateResearcher(item)}>
                {item.type != undefined && (
                  <div
                    className={`w-2 min-w-2 rounded-l-md dark:border-neutral-800 border min-h-[120px]  border-neutral-200 border-r-0 ${qualisColor[item.type as keyof typeof qualisColor]} min-h-full relative `}
                  ></div>
                )}
                  ...
                  <div className="flex w-full flex-col gap-1">
                    <div className="flex justify-between items-center">
                      <div className="text-xs font-medium mb-2 flex items-center gap-2">
                        {item.code != '' ? item.code : 'Sem código'}
                        {daysDifference <= 5 && (
