...
function DocenteStatsCards({ permanenteCount, colaboradorCount }: DocenteStatsCardsProps) {
    return (
...
function AddDocenteSection({ graduate_program_id, fetchDataAll }: AddDocenteSectionProps) {
    ...
    const handleAddInternal = async (e: FormEvent) => {
        ...
        try {
            const response = await fetch(`${urlGeralAdm}GraduateProgramResearcherRest/Insert`, {
                method: 'POST',
                mode: "cors",
                headers: {
                    "Access-Control-Allow-Origin": "*",
...
function DocenteListItem({ researcher, urlGeral, urlGeralAdm, onOpen, fetchDataAll }: DocenteListItemProps) {
    ...
    const initializeParticipations = (): Record<number, ParticipationType> => {
        ...
        const participationMap = new Map(researcher.participation.map(p => [p.year, p.type_ as ParticipationType]));
    ...
    const [participations, setParticipations] = useState(initializeParticipations());
    ...
    const getButtonVariant = (type: ParticipationType): 'default' | 'secondary' | 'outline' => {
    ...
    const handleSave = async () => {
        ...
        try {
            const deleteResponse = await fetch(`${urlGeralAdm}GraduateProgramResearcherRest/Delete`, {
                method: 'DELETE',
                mode: "cors",
                headers: {
                    "Access-Control-Allow-Origin": "*",
            ...
            if (deleteResponse.status !== 204) {
                toast.error("Erro ao deletar participações antigas. A atualização falhou.");
                setIsLoading(false);
                return;
            }
            ...
            const insertResponse = await fetch(`${urlGeralAdm}GraduateProgramResearcherRest/Insert`, {
                method: 'POST',
                mode: "cors",
                headers: {
                    "Access-Control-Allow-Origin": "*",
    ...
    return (
        <Alert>
            <AccordionItem value={researcher.researcher_id}>
                    ...
                    <div className="h-10">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <div className="text-xs text-gray-500">{researcher.lattes_id}</div>
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className=" items-center gap-3 hidden group-hover:flex transition-all">
                            <Button size={'icon'} onClick={() => onOpen('researcher-modal', { name: researcher.name })} variant={'ghost'} className="h-10 w-10 ">
                                <Maximize2 size={16} />
                            </Button>
                ...
                <AccordionContent>
                    <div className="pt-4">
                        <Label>Gerenciar participações (Últimas 2 quadrienais - Últimos 8 anos)</Label>
...
function DocenteList({ researchers, urlGeral, urlGeralAdm, onOpen, fetchDataAll }: DocenteListProps) {
    const [input2, setInput2] = useState('');

    const filteredTotal = Array.isArray(researchers) ? researchers.filter(item => {
