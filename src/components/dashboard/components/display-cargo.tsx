...
export function DisplayCargo(props: Disciplinas) {
  ...
  const handleSubmitFuncionalidade = async (permission: string) => {
    try {
      ...
      const urlProgram = urlGeralAdm + 's/permission';
  ...
  // Agrupar as funcionalidades por área
  const groupedByArea: FuncionalidadesAgrupadas = funcionalidadesData.reduce(
    (acc: FuncionalidadesAgrupadas, funcionalidade: Funcionalidade) => {
  ...
  const filteredList = sortedAreas
    .map((area) => {
      return {
        area,
  ...
  const urlPermission = urlGeralAdm + `s/permission?role_id=${props.id}`;
  ...
  return (
    <div className=" sticky top-8">
      ...
      ></div>
      <Alert className="p-0 rounded-t-none" x-chunk="dashboard-05-chunk-4">
            ...
            <ul className="grid gap-3">
              {Array.isArray(total) &&
                ...
                total.map((props) => (
                  <div
                    key={props.id}
                    className="group w-full flex justify-between gap-3 h-8 "
                  >
                    <p className="capitalize">
                      {props.permission.split('_').join(' ')}
                    </p>
                    <Button
                      onClick={() => handleDeleteFuncionalidade(props.id)}
                      variant={'destructive'}
                      size={'icon'}
                      className="w-8 h-8 group-hover:flex hidden"
                    >
                      <Trash className="h-3.5 w-3.5" />
                    </Button>
                  </div>
              ...
              <Dialog open={openPopo2} onOpenChange={setOpenPopo2}>
                ...
                <DialogContent className="z-[9999]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Todas as funcionalidades cadastradas no Módulo
                    ...
                    <div className="flex flex-col gap-1 p-2">
                      {filteredList.length > 0 ? (
                        filteredList.map((group, groupIndex) => (
                          <div key={groupIndex}>
                            <h3 className="text-gray-500 uppercase text-xs font-medium mb-2 mt-2">
                              {group.area.charAt(0).toUpperCase() +
                                group.area.slice(1)}
                            </h3>
        ...
        <CardFooter className="flex flex-row items-center border-t bg-muted/50 px-6 py-3">
          <div className="text-xs text-muted-foreground">Cargo | Conectee</div>
        </CardFooter>
      </Alert>
    </div>
  );
