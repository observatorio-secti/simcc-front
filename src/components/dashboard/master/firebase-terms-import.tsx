...
export function FirebaseTermsImport() {
  ...
  const importDataToFirebase = async (data: Csv[]) => {
    try {
      ...
      const existingTermValues = querySnapshot.docs.map(
        (doc) => doc.data().term,
      );
      ...
      for (const item of data) {
        ...
        const existingType = querySnapshot.docs.some(
          (doc) =>
            doc.data().term === item.term && doc.data().type_ === item.type_,
        );
        ...
        if (!existingTerm && !existingType) {
          ...
          console.log(
            `O termo "${item.term}" com o type_ "${item.type_}" já existe no banco de dados.`,
          );
        } else if (existingTerm) {
          console.log(`O termo "${item.term}" já existe no banco de dados.`);
        } else {
          console.log(
            `O type_ "${item.type_}" já existe para o termo "${item.term}" no banco de dados.`,
          );
        }
      }
  ...
  return (
    <div>
      <Button onClick={() => importDataToFirebase(filteredItems)}>
        Importar
      </Button>
    </div>
  );
