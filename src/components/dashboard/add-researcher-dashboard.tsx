...
export function AddResearcherDashboard() {
  ...
  const handleSubmitPesquisador = async () => {
    ...
    try {
      ...
      const urlProgram = urlGeralAdm + '/ResearcherRest/Insert';
      ...
      const fetchData = async () => {
        if (nomePesquisador.length != 0 && lattesID.length > 13) {
          ...
          if (
            nomePesquisador.length == 0 ||
            (lattesID.length == 0 && cpf.length == 0)
          ) {
            toast('Parece que os campos estão vazios', {
              description:
                'Preencha os campos nome do pesquisador e Lattes Id ou CPF',
              action: {
                label: 'Fechar',
                onClick: () => {},
              },
  ...
  const filteredTotal = Array.isArray(researcher)
  ...
  return (
    <>
      <Helmet>
        <title>Pesquisadores | Módulo administrativo | {'Simcc'} </title>
        <meta
          name="description"
          content={`Pesquisadores | Módulo administrativo | ${'Simcc'}`}
        />
          ...
          <div className="w-full mb-4  gap-4">
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="hidden items-center h-10 gap-2 md:ml-auto md:flex">
                {false && (
                  <Button size={'sm'} onClick={() => onOpen('import-docentes')}>
                    <FileXls size={16} />
          ...
          <div className="gap-4 md:gap-8 flex flex-col ">
            ...
            {has_editar_pesquisadores ? (
              <div className="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-4">
                ...
                <Alert
                  onClick={() => setIsOpenAdd(!onOpenAdd)}
                  className="p-0 hover:bg-eng-dark-blue bg-eng-blue dark:hover:bg-eng-dark-blue dark:bg-eng-blue text-white transition-all cursor-pointer "
                >
                  <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            {onOpenAdd && (
              <fieldset className="grid gap-6 rounded-lg  p-4 bg-white dark:border-neutral-800 border border-neutral-200 dark:bg-neutral-950 bg-cover  bg-center bg-no-repeat ">
                ...
                <div className="flex gap-3 items-end">
                  ...
                  <Button
                    onClick={() => handleSubmitPesquisador()}
                  ...
                  <Button
                    size={'icon'}
                    onClick={() => onOpen('add-researcher-csv')}
                    className="text-white dark:text-white"
                  >
                        ...
                        <div>
                                    ...
                                    <Alert>
                                      <div className="flex justify-between items-start gap-3">
                                        <div className="flex gap-3">
                                          <Avatar className="cursor-pointer rounded-md  h-10 w-10">
                                            <AvatarImage
                                              ...
                                              src={`${urlGeral}ResearcherData/Image?name=${item.name}`}
                                            />
                                        ...
                                        <div className="flex gap-2 items-center">
                                          <div
                                            className={` rounded-md h-4 w-4 ${item.status ? 'bg-green-500' : 'bg-red-500'}`}
                                          ></div>
                                      ...
                                      <div className="items-center flex justify-between gap-3">
                                        ...
                                        <div className="flex gap-3 justify-end w-full ">
                                          <Button
                                            onClick={() =>
                                              onOpen(
                                                'confirm-delete-researcher',
                                                {
                                                  id_delete: item.researcher_id,
                                                  name: item.name,
                                                },
                                              )
                                            }
                                          ...
                                          <Button
                                            onClick={() => {
                                              ...
                                              toast('Operação realizada', {
                                                description:
                                                  'ID Lattes copiado para área de transferência',
                                                action: {
                                                  label: 'Fechar',
                                                  onClick: () => {},
                                                },
                          ...
                          {filteredTotal.length > count && (
                            <div className="w-full flex justify-center mt-8">
                              <Button onClick={() => setCount(count + 24)}>
                                <Plus size={16} />
                                Mostrar mais
                              </Button>
                            </div>
