...
export const columns: ColumnDef<PosGraduationsProps>[] = [
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-2 items-center">
          {' '}
          <img
            className="w-4  object-cover object-center  whitespace-nowrap"
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center">
          <Hash size={12} /> {row.getValue('code')}
        </div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center">
          <GraduationCapIcon size={12} /> {row.getValue('type')}
        </div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center">
          <MapPin size={12} /> {row.getValue('city')}
        </div>
      );
    },
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="flex gap-1 items-center">
          <Star size={12} /> {row.getValue('rating')}
        </div>
      );
    },
  },

  {
    ...
    cell: ({ row }) => {
      ...
      const handleVisibleProgram = (id: string) => {
        const urlVisibleProgram =
          API_KEY + `GraduateProgramRest/Update?graduate_program_id=${id}`;
      ...
      return (
        <div className="flex gap-3 items-center">
          <Button
            onClick={() =>
              handleVisibleProgram(row.original.graduate_program_id)
            }
            variant="outline"
            className="ml-auto p-0  h-8 w-8 text-sm text-gray-500 dark:text-gray-300 flex items-center"
          >
            {row.original.visible ? <EyeSlash size={16} /> : <Eye size={16} />}
          </Button>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <Link to={`/pos-graducao/${row.original.code}`}>
              ...
              <DropdownMenuItem
                ...
                onClick={() =>
                  onOpen('edit-graduate-program', {
                    ...
                    menagers: row.original.menagers,
                  })
                }
              >
              ...
              <DropdownMenuItem
                onClick={() =>
                  onOpen('confirm-delete-pos-graduate-program', {
                    id_delete: row.original.graduate_program_id,
                    name: row.original.name,
                  })
                }
