...
export function GeralViewDashboard() {
  ...
  const urlDiretorio = `${urlGeralAdm}s/directory`;
    ...
    try {
      ...
      if (response.ok) {
        toast('Diretório alterado', {
          description:
            'Todos os arquivos necessários serão puxados dessa pasta',
          action: {
            label: 'Fechar',
            onClick: () => {},
          },
  ...
  const deleteItem = async (id: string) => {
    ...
    try {
      ...
      const q = query(
        collection(db, 'background_iapos'),
        where('id', '==', id),
      );
      ...
      querySnapshot.forEach(async (docSnapshot) => {
        ...
        const docRef = doc(db, 'background_iapos', docSnapshot.id);
        await deleteDoc(docRef);

        // Atualiza o estado para remover o item excluído
        setBackground((prevBackground) =>
          prevBackground.filter((item) => item.id !== id),
        );
  ...
  useEffect(() => {
    const fetchEmails = async () => {
      const querySnapshot = await getDocs(collection(db, 'background_iapos'));
  ...
  return (
    <div className="w-full relative">
      <Helmet>
        <title>Módulo administrativo | {'Simcc'} </title>
        <meta
          name="description"
          content={`Módulo administrativo | ${'Simcc'}`}
        />
      ...
      <main className="flex flex-1 flex-col gap-4  md:gap-8 ">
        <Tabs defaultValue={tab} value={tab} className="h-full">
              ...
              <div className="hidden items-center gap-2 md:ml-auto md:flex">
                <TabsList>
                  ...
                  <TabsTrigger
                    value="cargos"
                    disabled={
                      !has_editar_cargos_permissoes &&
                      !has_editar_cargos_usuarios &&
                      !has_editar_informacoes_usuarios
                    }
                  ...
                  <TabsTrigger
                    value="inst"
                    disabled={!has_editar_configuracoes_plataforma}
                    onClick={() => setTab('inst')}
                    className="text-zinc-600 dark:text-zinc-200"
                  >
                    Instituições
          ...
          <TabsContent value="all" className=" ">
            <div
              className={`p-4 md:p-8 pt-0 md:pt-0 h-auto flex flex-col gap-4 md:gap-8`}
            >
              <div
                ...
                >
                  <CardHeader className="flex gap-6 flex-col flex-wrap md:flex-row  justify-between">
                    ...
                    <div className="flex gap-3">
                      <Button onClick={() => onOpen('add-background')}>
                        <Plus size={16} />
                          ...
                          return (
                            <Alert className="flex justify-between group bg-neutral-100 border-0 dark:bg-neutral-800 dark:hover:bg-neutral-700 transition-all hover:bg-neutral-200  items-center">
                              <div className="flex items-center gap-3">
                                <div
                                  className="w-8 h-8 rounded-md whitespace-nowrap bg-cover bg-center bg-no-repeat "
                                  style={{
                              ...
                              <div className="flex gap-2 items-center">
                                ...
                                <Button
                                  ...
                                  variant={'ghost'}
                                  size={'icon'}
                                  className="h-8 w-8 transition-all group-hover:flex"
                                >
                                  <Pencil size={13} />
                                </Button>
                                <Button
          ...
          <TabsContent
            value="cargos"
            className="h-auto flex flex-col gap-8 m-0"
          >
            <CargosFuncoes />
          </TabsContent>
          ...
          <TabsContent
            value="unread"
            className="h-full flex flex-col gap-4 md:gap-8 px-4 md:px-8 m-0  "
          >
