...
export function MinhaArea() {
  ...
  const urlTermPesquisadores =
    urlGeral + `researcherName?name=${user?.researcger_name}`;
  ...
  const fetchDataPerm = async (role_id: string) => {
    const urlPermission = urlGeralAdm + `s/permission?role_id=${role_id}`;
  ...
  //permissoes
  const permissions = {
    hasBaremaAvaliacao: permission.some(
      (perm) => perm.permission === 'criar_barema_avaliacao',
    ),
    hasNotificacoes: permission.some(
      (perm) => perm.permission === 'enviar_notificacoes',
    ),
    hasVisualizarPesquisadores: permission.some(
      (perm) => perm.permission === 'visualizar_pesquisadores',
    ),
    hasVisualizarTodosDepartamentos: permission.some(
      (perm) => perm.permission === 'visualizar_todos_departamentos',
    ),
    hasVisualizarTodosProgramas: permission.some(
      (perm) => perm.permission === 'visualizar_todos_programas',
    ),
    hasVisualizarGruposPesquisa: permission.some(
      (perm) => perm.permission === 'visualizar_grupos_pesquisa',
    ),
    hasVisualizarInct: permission.some(
      (perm) => perm.permission === 'visualizar_inct',
    ),
    hasEditarPesosAvaliacao: permission.some(
      (perm) => perm.permission === 'editar_pesos_avaliacao',
    ),
    hasVisualizarIndicadoresInstituicao: permission.some(
      (perm) => perm.permission === 'visualizar_indicadores_instituicao',
    ),
    hasVisualizarGerenciaModuloAdministrativo: permission.some(
  ...
  const accessLinks = [
    {
      permission: 'hasVisualizarGerenciaModuloAdministrativo',
      to: '/dashboard/administrativo',
      icon: <SlidersHorizontal size={16} />,
      label: 'Administrativo',
    },
    {
      permission: 'hasVisualizarTodosDepartamentos',
      to: '/dashboard/departamentos',
      icon: <Building2 size={16} />,
      label: 'Departamentos',
    },
    {
      permission: 'hasVisualizarPesquisadores',
      to: '/dashboard/pesquisadores',
      icon: <Users size={16} />,
      label: 'Pesquisadores',
    },
    {
      permission: 'hasVisualizarTodosProgramas',
      to: '/dashboard/programas',
      icon: <GraduationCap size={16} />,
      label: 'Programas',
    },
    {
      permission: 'hasVisualizarGruposPesquisa',
      to: '/dashboard/grupos-pesquisa',
      icon: <Blocks size={16} />,
      label: 'Grupos de pesquisa',
    },
    {
      permission: 'hasVisualizarInct',
      to: '/dashboard/inct',
      icon: <FlaskConical size={16} />,
      label: "INCT's",
    },
    {
      permission: 'hasEditarPesosAvaliacao',
      to: '/dashboard/pesos-avaliacao',
      icon: <Weight size={16} />,
      label: 'Pesos de avaliação',
    },
    {
      permission: 'hasVisualizarIndicadoresInstituicao',
      to: '/dashboard/indicadores',
      icon: <PieChart size={16} />,
      label: 'Indicadores',
    },
    {
      permission: 'hasBaremaAvaliacao',
      to: '/dashboard/baremas',
      icon: <ClipboardEdit size={16} />,
      label: 'Baremas',
    },
    {
      permission: 'hasNotificacoes',
      to: '/dashboard/enviar-notificacoes',
      icon: <Mail size={16} />,
  ...
  return (
    ...
    >
      <SheetContent
        className={`p-0 dark:bg-neutral-900 dark:border-gray-600 ${expand ? 'w-full md:min-w-[80vw] lg:min-w-[80vw]' : 'lg:min-w-[50vw]'}`}
      >
        ...
        <div className="relative">
          <ScrollArea className="relative pb-4 whitespace-nowrap h-[calc(100vh-50px)] p-8 ">
            <div className="flex flex-col lg:flex-row gap-6 relative ">
                  ...
                  <div className="flex flex-col flex-1 w-full">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <DropdownMenu>
                        <div className={`   `}>
                          <DropdownMenuTrigger
                            className={`flex-1 items-center flex justify-center hover:bg-neutral-100 dark:hover:bg-neutral-800 transition-all w-fit rounded-md  `}
                          >
                            <Button
                              variant="outline"
                              role="combobox"
                        ...
                        <DropdownMenuContent className="min-w-[200px]  gap-1 flex flex-col ">
                          ...
                          {user?.roles != undefined &&
                            user.roles!.map((rola) => (
                              <DropdownMenuItem
                                className={`flex gap-2 items-center ${role == rola.role_id && 'bg-neutral-100 dark:bg-neutral-800'}`}
                                onClick={() => {
                                  fetchDataPerm(rola.id);
                                  localStorage.setItem(
                                    'role',
                                    JSON.stringify(rola.role_id),
                                  );
                    ...
                    <div
                      className={`flex flex-col lg:hidden lg:sticky w-full lg:w-fit mt-4  ${expand ? 'w-[240px]' : 'w-fit'}`}
                    >
                      {expand && (
                      ...
                      <div className="flex flex-col gap-2 mb-8">
                        <Button
                          onClick={() => setTab('all')}
                          variant={'ghost'}
                          className={`  ${!expand ? 'w-10' : 'justify-start'} ${tab == 'all' && 'bg-neutral-100 dark:bg-neutral-800'}`}
                        ...
                        <Button
                          onClick={() => setTab('seg')}
                          variant={'ghost'}
                          className={` ${!expand ? 'w-10' : 'justify-start'} ${tab == 'seg' && 'bg-neutral-100 dark:bg-neutral-800'}`}
                          size={expand ? 'default' : 'icon'}
                        >
                          <Shield size={16} />
                          {expand && 'Perfil e segurança'}
                        </Button>
                        {user?.researcger_name != '' && (
                          <Button
                            onClick={() => setTab('lin')}
                            variant={'ghost'}
                            className={` ${!expand ? 'w-10' : 'justify-start'} ${tab == 'lin' && 'bg-neutral-100 dark:bg-neutral-800'}`}
                            size={expand ? 'default' : 'icon'}
                          >
                            <ChartLine size={16} />
                            {expand && 'Linha do tempo'}
                          </Button>
                        )}
                      </div>

                      {expand &&
                        user?.graduate_program &&
                      ...
                      <div className="flex w-full flex-col gap-2 mb-8">
                        {user?.researcger_name != '' && (
                          <Button
                            variant={'ghost'}
                            className={` ${!expand ? 'w-10' : 'justify-start w-full'} ${tab == '' && 'bg-neutral-100 dark:bg-neutral-800'}`}
                            size={expand ? 'default' : 'icon'}
                          >
                            <Cube size={16} />
                            {expand && 'Meus bens patrimoniados'}
                          </Button>
                        )}
                        {user?.researcger_name != '' && (
                          <Link
                            className="w-full"
                            target="_blank"
                            to={`/researcher?researcher_name=${user?.researcger_name}&search_type=&terms=`}
                          >
                            <Button
                              variant={'ghost'}
                              className={` ${!expand ? 'w-10' : 'justify-start w-full'} ${tab == '' && 'bg-neutral-100 dark:bg-neutral-800'}`}
                            ...
                            >
                              <User size={16} />
                              {expand && 'Página pública do pesquisador'}
                            </Button>
                          </Link>
                        )}
                      </div>
                      ...
                      <div className="flex flex-col gap-2">
                        <Button
                          ...
                          variant={'destructive'}
                          className={` ${!expand ? 'w-10' : 'justify-start'} ${tab == '' && 'bg-neutral-100 dark:bg-neutral-800'}`}
                          size={expand ? 'default' : 'icon'}
                        >
                          <SignOut size={16} />
                          {expand && 'Encerrar sessão'}
                        </Button>
                      </div>
                    </div>
                    ...
                    {researcher &&
                      researcher.slice(0, 1).map((props) => {
                        ...
                        const monthDifference =
                          (currentDate.getFullYear() - lattesYear) * 12 +
                          (currentDate.getMonth() + 1 - lattesMonth);
                            ...
                            <div className="bg-red-50 flex gap-3 dark:bg-red-200/20 w-full mt-6 p-8 rounded-md">
                              ...
                              <div>
                                <AlertTitle className="whitespace-normal">
                                  Currículo Lattes desatualizado
                                </AlertTitle>
                                <AlertDescription className="whitespace-normal mb-6">
                                  Seu currículo na plataforma Lattes está sem
                                  alterações tem {monthDifference} meses. Última
                                  atualização em {String(props.lattes_update)}.
                                </AlertDescription>

                                <Link
                                  to={'https://lattes.cnpq.br/'}
                                  target="_blank"
                                >
                                  {' '}
                    ...
                    <div
                      className={`flex flex-col md:grid gap-4  ${expand ? 'grid-cols-1 md:grid-cols-2 xl:grid-cols-3' : 'md:grid-cols-1 xl:grid-cols-2 lg:grid-cols-1'}`}
                    >
                      {accessLinks.map(
                        ({ permission, to, icon, label }) =>
              ...
              <div
                className={`hidden lg:flex flex-col lg:sticky  w-full lg:w-fit top-8 ${expand ? 'w-[240px]' : 'w-fit'}`}
              >
                {expand && (
                  <p className="text-gray-500 uppercase text-xs font-medium mb-2">
                    PÁGINAS
